def title_shape_config(global_dim: list, # in number of elements
                       is_mx: bool,
                       datatype_bit: int = 8 # 32/16/8/4
                       ):
    subtile_dim = [0] * 2 # Sub-Tile shape [K, M], in elements
    tile_dim = [0] * 2 # Tile shape [K, M], in elements
    supertile_dim = [0] * 2 # Supertile shape [K, M], in tiles
    ...
    if datatype_bit != 6:
        ...
        subtile_dim = [8, int(subtile_data_byte // 8 // (float(datatype_bit) / 8))]
        tile_data_byte = num_subtile_in_tile * subtile_data_byte # only for data. Header byte not included
    ...
    for dim in range(2):
        if global_dim[dim] % tile_dim[dim] != 0:
            ...
            print("tile-level padding: dim {} pad {} elements".format(dim, tile_padding_elements[dim]))

    # supertile-level padding
    for dim in range(2):
        global_dim_tiles = (global_dim[dim] + tile_padding_elements[dim]) // tile_dim[dim]
        if global_dim_tiles % supertile_dim[dim] != 0:
            supertile_padding_tiles[dim] = supertile_dim[dim] - global_dim_tiles % supertile_dim[dim]
            print("supertile-level padding: dim {} pad {} tiles.".format(dim, supertile_padding_tiles[dim]))
