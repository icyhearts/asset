...
def _qdq_unaligned_matmul_activation_qdq_model(
  tmp_path, input_shape, input_config, shape_id=None
):
  ...
  if shape_id is None:
    if input_shape is None:
      shape_id = "unknown_rank"
    elif any(isinstance(dim, str) for dim in input_shape):
      shape_id = "symbolic"
    else:
      shape_id = "static"
...
@pytest.mark.parametrize(
  "input_shape",
  ([2, 3, 18], ["B", "S", 18], None),
  ids=("static", "symbolic", "unknown_rank"),
)
@pytest.mark.parametrize("dtype", ("mxint8", "int8"))
def test_unaligned_matmul_activation_qdq_matches_simo_torch_with_padding(
  tmp_path, input_shape, dtype, shape_id=None
):
  session = _qdq_unaligned_matmul_activation_qdq_model(
    tmp_path, input_shape, {"dtype": dtype}, shape_id
  )
