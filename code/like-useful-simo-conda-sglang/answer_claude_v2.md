...
## 64. `test_bf16_mxi8_host.cpp` 逐行讲解：`sipu::` 符号的来源、`mma_dte` 的实现位置，以及 device code 是怎么链进可执行文件的

**问题**（四个）：

1. 逐行讲解 `testcase/func_test/test_bf16_mxint8_host/test_bf16_mxi8_host.cpp`。
2. 这份 `.cpp` 用到的所有 `sipu::` 命名空间下的类型和函数（`make_shape` / `make_tensor` / `make_layout` …）定义在哪里？
3. `mma_dte<InputT, OutputT, OutputT, LayoutA, LayoutB, LayoutC>` 在本例子中的实现位于哪里？
4. 编译成可执行文件时，CMake 是怎么把 device code 链上去的 —— 是**先把整个主库的 device code 编成一个 `.so` 再 `target_link_libraries`**，还是**只把这份 `.cpp` 用到的那几个 device code 编成 `.o` 再一起链接**？产物里是**全部 device 实现**还是**只调用的那几个**？

**一句话结论（第 4 问，先讲，因为它最容易被想反）：两者都不是。** 真实路径是第三条：

- 这份 test 目录下有一份**自己的 `.su`**（`testcase/func_test/test_bf16_mxint8_host/kernel/inst_bf16_mxi8.su`），它只做 **2 个 `mma_dte` 特化的显式实例化**；
- 它由 `scc` 编成**一个 `.o`**，再被 `/usr/bin/c++ -shared` 打成一个**私有的小 `.so`**：`libtile_mma_dte_mxint8.so`（5.4 MB）；
- 可执行文件 `test_bf16_mxi8_host`（542 KB）通过 `-ltile_mma_dte_mxint8` 链接这个 `.so`，**可执行文件里一个字节的 device code 都没有**（没有 `fatbin`、没有 `kernel_meta` 段 —— 见 64.4.2 的 `readelf` 实测）。
- **而那个 `.so` 里装的不是"少数几个 device 实现"，是 mxint8 这一族几乎所有的实现：4214 个 device kernel 特化。** 因为显式实例化会连带把整个 dispatch 机制（`MMA_DTE_SINGLE_FAMILY_DISPATCH=1` + `dispatch_cases_mxint8_generated.hpp`）全部拉进来。**"只实例化用到的 2 个模板" ≠ "只生成 2 个 device kernel"。**

另外要纠正一个常见误解：**这条路径完全没有碰生产库（主库）**。在 `testcase/` 下 74 个 testcase 项目里，**只有 4 个**真正调用 `mma_dte_get_main_object()` 去复用生产 `.o`（该函数定义在 `testcase/func_test/CMakeMmaDteCommon.cmake:209`）：`test_bf16_fp16_r32_host`、`test_bf16_fp16_fp32_r32_host`、`test_bf16_mxfp4_r32_host`、`test_int32_int8_r32_host`。`test_bf16_mxint8_host` **不在其中** —— 它的 `CMakeLists.txt` 里没有任何 `mma_dte_get_main_object` / `MMA_DTE_MAIN_BUILD_DIR` 引用（下面 64.4.4 有对照）。

产物结构总览：

| 产物 | 大小 | 含 device code？ | 来源 |
| --- | --- | --- | --- |
| `test_bf16_mxi8_host`（可执行文件） | 542 KB | **否**（无 `fatbin` 段） | `test_bf16_mxi8_host.cpp` → `/usr/bin/c++` |
| `libtile_mma_dte_mxint8.so` | 5.4 MB | **是**，4.2 MB `__sipu_fatbin` + 180 KB `.sipu_kernel_meta` | `kernel/inst_bf16_mxi8.su` → `scc` → `/usr/bin/c++ -shared` |
| 生产库 `libtile_mma_dte_<TYPE>.so`（**本 test 未使用**） | — | 是 | `kernel/instantiations/*/inst_*.su` → `scc`（40 个） |

（`libtile_mma_dte_<TYPE>.so` 的数量以本次构建为准：`kernel/instantiations/` 下共 **40** 个 `inst_*.su`；`cmake.log:9` 里同时出现的 **74** 是 testcase 项目数，两个数不是一回事。）
...
| 问题 | 结论 | 关键位置 |
| --- | --- | --- |
| Q1 逐行讲解 | 见 64.1 | `testcase/func_test/test_bf16_mxint8_host/test_bf16_mxi8_host.cpp:1-147` |
| Q2 `sipu::` 符号在哪 | **全在 SDK 的 `SiTe` 模板库里，不在 sikernel 仓库里** | `$SI_SDK_ROOT/include/SiTe/tensor/tensor.hpp:38`（`namespace sipu`） |
| Q3 `mma_dte` 实现在哪 | 模板声明在 `include/simma.h`，实现全在 `kernel/mma_dte_tiled_tensor.hpp`，真正被编译的是本 test 自己的 `.su` 显式实例化 | `include/simma.h:179`、`kernel/mma_dte_tiled_tensor.hpp:193`、`testcase/.../kernel/inst_bf16_mxi8.su:24` |
| Q4 device code 怎么链 | 本 test 的 `.su` → `scc` → 私有 `.so`；可执行文件只链 `.so`，**本身不含 device code** | `testcase/func_test/test_bf16_mxint8_host/CMakeLists.txt:43-79` |

---

### 64.1 `test_bf16_mxi8_host.cpp` 逐行讲解

先说这个例子在干什么：它把 **mxint8 输入的 A/B**（在 host 上用 gold 路径算出 float 参考解），再把 A/B 拷到设备上，调用 `mma_dte<mxint8, bf16|fp16, ...>` 做一次 `A × Bᵀ`，最后逐元素比对。里面有两个 case：**bf16 输出 / R8 通路** 和 **fp16 输出 / R32 通路**。

#### 64.1.1 头部与 layout 常量（`test_bf16_mxi8_host.cpp:1-37`）
...
| 行 | 代码 | 说明 |
| --- | --- | --- |
| `1` | `#include <gtest/gtest.h>` | 唯一的测试框架依赖 |
| `19` | `#include <sipu_runtime.h>` | 运行时 API：`sipuMalloc` / `sipuMemcpy` / `sipuFree` |
| `20` | `#include "sipu.h"` | 提供 `sipu::` 命名空间的入口头（见 64.2） |
| `21` | `#include "SiTe.hpp"` | **关键**：它把 `SiTe/tensor/tensor.hpp` 带进来，`sipu::make_shape` 等全在这里面 |
| `29` | `#include "simma.h"` | `mma_dte` 的模板**声明**（sikernel 仓库自己的头，`include/simma.h`） |
| `24` | `#include <filesystem>` | 本文件里其实没用到，冗余包含 |

Layout 常量（`test_bf16_mxi8_host.cpp:33-37`），构造自 `mma_dte_api::tensor_layout(tile_dim0, tile_dim1, supertile_shape0, supertile_shape1, tensor_format)`（定义在 `include/simma.h:41`，构造函数在 `:47`）：

| 行 | 常量 | 值 | `tensor_format` |
| --- | --- | --- | --- |
| `33` | `kLayoutB` | `(32, 32, 4, 1, 1)` | 1 = Tiled |
| `34` | `kLayoutA_R8` | `(128, 8, 4, 1, 1)` | 1 = Tiled |
| `35` | `kLayoutC_R8` | `(64, 8, 1, 1, 0)` | **0 = Linear** |
| `36` | `kLayoutA_R32` | `(32, 32, 4, 1, 1)` | 1 = Tiled |
| `37` | `kLayoutC_R32` | `(16, 32, 1, 1, 1)` | 1 = Tiled |

注意 `33`/`34`/`36` 的 `tile_dim1` 分别是 `32`/`8`/`32` —— 这三个数**决定了最终走哪条 kernel 通路**。`kernel/mma_dte_tiled_tensor.hpp:102-117` 的 `if constexpr (layoutA.tile_dim1 == 32) → mma_r32_dte / == 16 → mma_r16_dte / else → mma_r8_dte` 就是按它分派的。所以 `kLayoutA_R8`（`tile_dim1=8`）走 **r8**，`kLayoutA_R32`（`tile_dim1=32`）走 **r32**。

#### 64.1.2 `run_case()` —— 模板参数与准备（`test_bf16_mxi8_host.cpp:39-50`）

```cpp
39  template <class OutputT, int CaseM, int CaseN, int CaseK,
40            mma_dte_api::tensor_layout LayoutA,
41            mma_dte_api::tensor_layout LayoutB,
42            mma_dte_api::tensor_layout LayoutC>
43  static int run_case() {
44      using InputT = sifmt::mxint8;
```

| 行 | 代码 | 说明 |
| --- | --- | --- |
| `39-42` | 模板参数 | `OutputT` 是**输出类型**；`LayoutA/B/C` 是**非类型模板参数**（C++20 的结构化类型 NTTP），所以 `kLayoutA_R8` 这种 `constexpr auto` 能直接当模板实参传 |
| `44` | `using InputT = sifmt::mxint8;` | 输入类型固定为 mxint8。`sifmt::mxint8` 的定义在 SDK：`$SI_SDK_ROOT/include/SiTe/sifmt/sifmt_mx.hpp:115` = `SiMxData<sifmt::int8, mxi8::MxQuantPolicy<>>` |
| `45-50` | 随机 A | `std::uniform_real_distribution<float>(-100, 100)`；A 的**元素个数**是 `CaseM * CaseK`，注意这里填的是**逻辑形状**，不是 padded 后的存储形状 |

#### 64.1.3 A / B 的 host 端构造（`test_bf16_mxi8_host.cpp:51-61`）

```cpp
51      auto shape_a = sipu::make_shape(CaseM, CaseK);
52      auto layout_a = sipu::make_layout(shape_a, sipu::LayoutTag::Tiled);
53      auto mx_tensor_a = sipu::make_tensor<InputT>(layout_a, arr_A);
```

| 行 | 代码 | 说明 |
| --- | --- | --- |
| `51` | `make_shape(CaseM, CaseK)` | 造一个 2 维 `Shape<2>` |
| `52` | `make_layout(shape, LayoutTag::Tiled)` | 造 Tiled layout |
| `53` | `make_tensor<InputT>(layout, arr_A)` | **这一步在 host 上就把 float→mxint8 量化 + 摆成 tiled 存储** 做完了（SDK 的 `Tensor` 走的是 `std::span<const float>` 那条构造） |
| `59-61` | 同 B | B 也是 mxint8 / Tiled |

**注意 A 和 B 在算术上的形状**：`shape_a = (M, K)`，`shape_b = (N, K)` —— B 用的也是 `(N, K)` 而不是常见的 `(K, N)`，因为后面 `mm_mnk` 和 `mma_dte` 算的都是 **`A × Bᵀ`**（`mma_dte_api::Operation::OP_N`）。

#### 64.1.4 golden 参考解（`test_bf16_mxi8_host.cpp:63-72`）

```cpp
63      auto gold_tiled_tensor =
64          sipu::tensor::mm_mnk(mx_tensor_a, mx_tensor_b, sipu::LayoutTag::Tiled);
65      auto gold_linear = gold_tiled_tensor.toVector();
66      auto shape_d = sipu::make_shape(CaseM, CaseN);
67      auto layout_d = sipu::make_layout(shape_d, sipu::LayoutTag::Tiled);
68      auto out_tensor_d = sipu::make_tensor<OutputT>(layout_d, gold_linear);
69      auto gold_tiled = out_tensor_d.toVectorAsMemoryOrder(true);
```

| 行 | 代码 | 说明 |
| --- | --- | --- |
| `63-64` | `tensor::mm_mnk(...)` | **host 上的参考矩阵乘**（把 mxint8 反量化成 float 再乘）。注意它**不是** device kernel，是纯 CPU 的循环；`sipu::tensor` 这个子命名空间在 SDK 里于 `tensor.hpp:2534` 打开 |
| `65` | `.toVector()` | 结果摊成 `std::vector<float>`，**按逻辑顺序** |
| `66-67` | 造 D 的 shape/layout | D 是 `(M, N)` |
| `68` | `make_tensor<OutputT>(layout_d, gold_linear)` | 把 float 参考解 **转成 `OutputT`（bf16/fp16）并按 Tiled 摆放** |
| `69` | `.toVectorAsMemoryOrder(true)` | 取出**按物理内存顺序**排列的 `gold_tiled` |

**`70-72` 是理解整个对比逻辑的关键**：

```cpp
70      const size_t output_elements =
71          LayoutC.tensor_format == 1 ? gold_tiled.size() : gold_linear.size();
72      const size_t size_out = output_elements * sizeof(OutputT);
```

- `LayoutC.tensor_format == 1`（tiled 输出）→ 用 `gold_tiled`（物理序），元素个数取 `gold_tiled.size()`；
- `LayoutC.tensor_format == 0`（linear 输出）→ 用 `gold_linear`（逻辑序）。

对应到两个 case：`kLayoutC_R8 = tensor_layout(64,8,1,1,0)` 的 `tensor_format=0` → 走 linear 分支；`kLayoutC_R32 = tensor_layout(16,32,1,1,1)` 的 `tensor_format=1` → 走 tiled 分支。**同一条 评判代码，靠这个分支同时覆盖两种输出排布。**

#### 64.1.5 设备内存与调用（`test_bf16_mxi8_host.cpp:74-99`）

```cpp
80      size_t tensor_size_a = mx_tensor_a.storageSize();
81      size_t tensor_size_b = mx_tensor_b.storageSize();
83      sipuMalloc(&d_A, tensor_size_a);
...
87      sipuMemcpy(d_A, mx_tensor_a.data(), tensor_size_a, sipuMemcpyHostToDevice);
```

| 行 | 代码 | 说明 |
| --- | --- | --- |
| `74` | `malloc(size_out)` | host 侧收结果的 buffer（注意是 `malloc` 不是 `sipuMalloc`） |
| `80-81` | `.storageSize()` | **物理存储字节数**（padded + tile header 后的），不是 `M*K*sizeof`。这就是要拷多少字节的依据 |
| `83-85` | `sipuMalloc` | 设备侧三块：`d_A`、`d_B`、`d_C` |
| `87-88` | `sipuMemcpy(..., HostToDevice)` | `.data()` 返回的是 **host 侧**那块的指针 |

核心调用（`test_bf16_mxi8_host.cpp:90-94`）：

```cpp
90      mma_dte<InputT, OutputT, OutputT, LayoutA, LayoutB, LayoutC>(
91          mma_dte_api::Operation::OP_N, mma_dte_api::Operation::OP_N,
92          CaseM, CaseN, CaseK, OutputT(0.1f), static_cast<void*>(d_A), CaseK,
93          static_cast<void*>(d_B), CaseK, OutputT(0.2f),
94          static_cast<void*>(d_C), CaseN);
```

- `90` 模板实参依次是 `inputT=mxint8`、`outputT=OutputT`、`scalarT=OutputT`（第 3 个），再是三个 layout —— 和 `include/simma.h:146` 的声明一一对应。
- `91` `transa` / `transb` 都是 `OP_N`。
- `92` 传 `alpha=OutputT(0.1f)`、`lda=CaseK`；`93` 传 `beta=OutputT(0.2f)`、`ldb=CaseK`；`94` 传 `ldc=CaseN`。
- **这里选的是 13 个参数、不带 workspace 的那条重载**（`include/simma.h:179-187`），`stream` 走默认 `nullptr`。

> ⚠️ **`alpha` / `beta` 实际上被忽略了**，所以这个例子拿纯 `A×B`（不带 `0.1*` / `+0.2*C`）的 golden 去比也能过。证据在实现里：`kernel/mma_dte_tiled_tensor.hpp:69` 明写着
> ```cpp
> 69    // TODO: Use transa, transb, alpha, beta, lda, ldb, ldc in implementation
> ```
> 同理 `transa`/`transb`/`lda`/`ldb`/`ldc` 也都没被真正使用。**读这个例子时不要把 `0.1f`/`0.2f` 当成有效语义**。

收尾（`test_bf16_mxi8_host.cpp:96-99`）：`sipuMemcpy(host_C, d_C, size_out, DeviceToHost)` 然后三个 `sipuFree`。

#### 64.1.6 校验与两个 case（`test_bf16_mxi8_host.cpp:101-146`）

```cpp
102     for (size_t i = 0; i < output_elements; ++i) {
103         OutputT gold;
104         if constexpr (LayoutC.tensor_format == 1) {
105             gold = static_cast<OutputT>(gold_tiled[i]);
106         } else {
107             gold = OutputT(gold_linear[i]);
108         }
```

| 行 | 说明 |
| --- | --- |
| `104-108` | 用 `if constexpr` 在**编译期**选 gold 来源，和 `70-71` 的 `output_elements` 完全对应 |
| `109-113` | 相对误差 `err_rate = |(gold-result)/gold|`，`gold==0` 时退化为绝对误差 |
| `114-119` | **`err_rate > 0.0f` 就算错** —— 也就是说这里要求**逐 bit 完全一致**，没有任何容差 |
| `126-129` | `run_bf16_default_case()`：`<bf16, 8, 576, 1024, kLayoutA_R8, kLayoutB, kLayoutC_R8>` → M=8, N=576, K=1024，走 r8 |
| `131-134` | `run_default_case()`：`<fp16, 32, 576, 1024, kLayoutA_R32, kLayoutB, kLayoutC_R32>` → M=32, 走 r32 |
| `136-146` | 两个 `TEST_F`，各自 `EXPECT_EQ(run_*_case(), 0)` |

---

### 64.2 Q2：`sipu::` 下的类型和函数定义在哪里？

**全部在 SDK 里，不在 sikernel 仓库里。** sikernel 只用一个**同名命名空间**的 `check()`（在 `$SI_SDK_ROOT/include/deprecated.h:78`，由 `$SI_SDK_ROOT/include/sipu.h:57` 的 `#include "deprecated.h"` 引入）。注意 `sipu.h` 里那个 `namespace sipu` 只开在 `:24-51`，且里面**只有 3 个 `__in_simulator__` / `__in_device__` / `__sipu_arch__` 的小函数**，`make_shape` 之类**一个字都没有** —— 它们全在 `SiTe/tensor/tensor.hpp` 里另开的同名命名空间中。

命名空间本体在 **`$SI_SDK_ROOT/include/SiTe/tensor/tensor.hpp`**，`namespace sipu {` 开在 **`:38`**，闭在 **`:2765`**（其中嵌套的 `namespace detail` 在 `:373-441`，`namespace tensor` 在 `:2534-2763`）。

| 例子里的用法（行号） | 定义位置 | 函数名 / 类型名 |
| --- | --- | --- |
| `sipu::make_shape`（`:51`, `:59`, `:66`） | `SiTe/tensor/tensor.hpp:343` | `make_shape` |
| `sipu::LayoutTag::Tiled`（`:52`, `:60`, `:64`, `:67`） | `SiTe/tensor/tensor.hpp:473` | `LayoutTag`（枚举 `Manual` / `Linear` / `Tiled`） |
| `sipu::make_layout`（`:52`, `:60`, `:67`） | `SiTe/tensor/tensor.hpp:597` | `make_layout(Shape_, LayoutTag)`（另有 `:591` 的 stride 版、`:603` 的 tiling-config 版） |
| `sipu::make_tensor<InputT>`（`:53`, `:61`, `:68`） | `SiTe/tensor/tensor.hpp:2483` | `make_tensor(Layout_, T1)`（另有 `:2489` / `:2495` / `:2504` 重载） |
| `sipu::tensor::mm_mnk`（`:64`） | `SiTe/tensor/tensor.hpp:2679` | `mm_mnk`（在 `namespace tensor`，`:2534` 开） |
| `.toVector()`（`:65`） | `SiTe/tensor/tensor.hpp:2114` | `Tensor::toVector` |
| `.toVectorAsMemoryOrder(true)`（`:69`） | `SiTe/tensor/tensor.hpp:2141` | `Tensor::toVectorAsMemoryOrder` |
| `.storageSize()`（`:80`, `:81`） | `SiTe/tensor/tensor.hpp:2238` | `Tensor::storageSize` |
| `.data()`（`:87`, `:88`） | `SiTe/tensor/tensor.hpp:2243` | `Tensor::data` |
| `Tensor` 类本体 | `SiTe/tensor/tensor.hpp:1755` | `Tensor` |
| `sifmt::mxint8`（`:44`） | `SiTe/sifmt/sifmt_mx.hpp:115` | `mxint8`（= `SiMxData<sifmt::int8, mxi8::MxQuantPolicy<>>`） |
| `sifmt::bfloat16`（`:127`） | `SiTe/sifmt/sifmt_fp.hpp:278` | `bfloat16` |
| `sifmt::float16`（`:132`） | `SiTe/sifmt/sifmt_fp.hpp:277` | `float16` |

运行时 API 也在 SDK：

| 用法 | 定义位置 | 函数名 |
| --- | --- | --- |
| `sipuMalloc`（`:83-85`） | `include/sipu_runtime.h:560` | `sipuMalloc<T>(T**, size_t)` —— 是个模板包装，转调 `::sipuMalloc((void**), size)`（`:562`） |
| `sipuMemcpy`（`:87`, `:88`, `:96`） | `include/sipurt/sipu_runtime_api.h:4661` | `sipuMemcpy` |
| `sipuMemcpyHostToDevice` / `MemcpyDeviceToHost` | 同文件，枚举 `sipuMemcpyKind` | — |
| `sipuFree`（`:97-99`） | `include/sipurt/sipu_runtime_api.h:3752` | `sipuFree` |

`make_shape` / `make_tensor` / `make_layout` 都带 `__SITE_SIFMT_HOST__ __SITE_SIFMT_DEVICE__` 宏（例如 `tensor.hpp:343`、`:2483`），所以**同一份代码 host 和 device 都能用**。

---

### 64.3 Q3：`mma_dte<InputT, OutputT, OutputT, LayoutA, LayoutB, LayoutC>` 的实现位置

**声明**（sikernel 仓库，`include/simma.h`）—— 三条重载：

| 行 | 签名 |
| --- | --- |
| `include/simma.h:146-156` | 带 `void* workspace, size_t workspaceSizeBytes` 的版本 |
| `include/simma.h:179-187` | 不带 workspace 的版本（**本 test 用的就是这条**） |
| `include/simma.h:201-203` | 简化版 `mma_dte(void* A, void* B, void* C, int M, int N, int K, stream)` |

**实现**（`kernel/mma_dte_tiled_tensor.hpp`），调用链如下：

```
mma_dte<...>()                                    kernel/mma_dte_tiled_tensor.hpp:193   ← 本 test 命中
  └─ mma_dte_implement<...>()                     kernel/mma_dte_tiled_tensor.hpp:165
       └─ mma_dte_implement<...>()                kernel/mma_dte_tiled_tensor.hpp:151   ← 带 workspace 的重载
            └─ mma_dte_implement_with_control<...>()   kernel/mma_dte_tiled_tensor.hpp:54
                 ├─ sipu::check(...)      (5 处 shape/layout 校验，:71-87)
                 ├─ get_best_solution<...>()        (:88-90)      → planner 搜索最优配置
                 ├─ create_tensor_maps<...>()       (:91-97)      → 构造 A/B/D 的 TensorMap
                 └─ if constexpr (layoutA.tile_dim1)  (:102-117)  ← 路由到 r32 / r16 / r8
                      ├─ mma_r32_dte   kernel/detail/mma_dte_tiled_tensor_dispatch_entrypoints.hpp:1436
                      ├─ mma_r16_dte   kernel/detail/mma_dte_tiled_tensor_dispatch_entrypoints.hpp:1321
                      └─ mma_r8_dte    kernel/detail/mma_dte_tiled_tensor_dispatch_entrypoints.hpp:1211
```

对两个 case：

- `kLayoutA_R8` 的 `tile_dim1 == 8` → `else` 分支 → `mma_r8_dte`（`dispatch_entrypoints.hpp:1211`）；由 `.su:18` 的 `mma_dte_tiled_tensor_kernel_util_mxint8_mxfp6_r8.hpp` 提供 r8 的 device 实现（该头 6216 行，实际 kernel 本体）。
- `kLayoutA_R32` 的 `tile_dim1 == 32` → `mma_r32_dte`（`dispatch_entrypoints.hpp:1436`）；由 `.su:18` 的 `..._r32.hpp` 提供。

**但是真正被编译器实例化的是哪一份？** 是 **test 目录下自己的 `inst_bf16_mxi8.su`**：

```cpp
// testcase/func_test/test_bf16_mxint8_host/kernel/inst_bf16_mxi8.su:18-36
18  #include "util/mma_dte_tiled_tensor_kernel_util_mxint8_mxfp6_r32.hpp"
19  #include "util/mma_dte_tiled_tensor_kernel_util_mxint8_mxfp6_r16.hpp"
20  #include "util/mma_dte_tiled_tensor_kernel_util_mxint8_mxfp6_r8.hpp"
21  #include "mma_dte_tiled_tensor.hpp"
24  template void mma_dte<sifmt::mxint8, sifmt::bfloat16, sifmt::bfloat16,
25                        mma_dte_api::tensor_layout(128,8,4,1,1),
26                        mma_dte_api::tensor_layout(32,32,4,1,1),
27                        mma_dte_api::tensor_layout(64,8,1,1,0)>( ... );
31  template void mma_dte<sifmt::mxint8, sifmt::float16, sifmt::float16,
32                        mma_dte_api::tensor_layout(32,32,4,1,1),
33                        mma_dte_api::tensor_layout(32,32,4,1,1),
34                        mma_dte_api::tensor_layout(16,32,1,1,1)>( ... );
```

**这 2 个 `template void` 就是 `.cpp:90` 调用的那 2 个特化。** `.cpp` 侧只有**声明**（`include/simma.h`），符号是 `U`（undefined），由这个 `.so` 提供。

对照：生产库的同一族 `kernel/instantiations/mxint8/*.su` 每个文件有 **12** 个 `template void mma_dte`（`inst_mxint8_r16.su` / `_r32_1x4` / `_r32_2x2` / `_r32_4x1` / `_r8` 共 5 个文件、60 个特化），本 test 只要 **2** 个。

---

### 64.4 Q4（核心）：device code 是怎么链进可执行文件的？

#### 64.4.1 CMake 侧的三个步骤

`testcase/func_test/test_bf16_mxint8_host/CMakeLists.txt`：

**第一步（`:43-47`）—— 用 `scc` 把 `.su` 编成 object library：**

```cmake
43  scc_add_library(tile_mma_dte_mxint8 OBJECT
44      ${TEST_INST_SRC}
45  )
46  scc_target_compile_options(tile_mma_dte_mxint8 ${COMMON_COMPILE_OPTIONS})
47  scc_target_include_directories(tile_mma_dte_mxint8 ${COMMON_INCLUDE_DIRECTORIES})
```

`TEST_INST_SRC` 在 `:41` 定义（默认指向 `kernel/inst_bf16_mxi8.su`）。`scc_add_library` 由 `find_package(scc REQUIRED)` 引入，实现在 SDK：`$SI_SDK_ROOT/share/cmake/scc/sccConfig.cmake:315`。

**第二步（`:52-62`）—— 用 host 编译器把它打成一个私有 `.so`：**

```cmake
52  add_custom_command(
53      OUTPUT ${CMAKE_CURRENT_BINARY_DIR}/libtile_mma_dte_mxint8.so
54      COMMAND ${CMAKE_CXX_COMPILER} -shared -o ${CMAKE_CURRENT_BINARY_DIR}/libtile_mma_dte_mxint8.so
55          ${TILE_MMA_DTE_MXINT8_O}
56      DEPENDS tile_mma_dte_mxint8 ${TILE_MMA_DTE_MXINT8_O}
57      COMMENT "Linking libtile_mma_dte_mxint8.so"
58  )
```

**第三步（`:67-81`）—— 可执行文件只链这个 `.so`：**

```cmake
67  add_executable(test_bf16_mxi8_host
68      ${TEST_HOST_SRC}
69  )
74  target_link_libraries(test_bf16_mxi8_host PRIVATE
75      ${CMAKE_CURRENT_BINARY_DIR}/libtile_mma_dte_mxint8.so
76      sipurt
77      sipu
78  )
79  add_dependencies(test_bf16_mxi8_host tile_mma_dte_mxint8_so)
81  mma_dte_test_enable_gtest(test_bf16_mxi8_host)
```

`sipurt` / `sipu` 是 SDK 的运行时导入库（`$SI_SDK_ROOT/lib/libsipurt.so.0.4.1`、`libsipu.so.0.4.1`）。

**所以：不是"整个主库编成 `.so` 再链"，而是"本 test 自己的 `.su` 编成 `.so` 再链"。** 生产库的 40 个 `libtile_mma_dte_<TYPE>.so` **跟这个 test 一次都没关系**。

#### 64.4.2 实测：可执行文件里没有 device code

`temp/cxx.log:6116-6142` 记录的完整命令（`build-modify.sh --test`，SDK tag `2609161442`）：

```
6117  .../bin/scc -arch=sipu_150 -c
        -o .../_SipuObj/tile_mma_dte_mxint8/kernel/inst_bf16_mxi8.su.o
        .../testcase/func_test/test_bf16_mxint8_host/kernel/inst_bf16_mxi8.su
        -I.../include -I.../include/SiTe -I.../include/sipurt -I.../include -I.../kernel -I.../kernel/util
        --clangopt=-DSIRT_DEVICE_USE_PRINTF --clangopt=-fPIC --clangopt=-DDTE_DEV_MODE
        --clangopt=-DMMA_DTE_TEST_GRID_DIM=16 --clangopt=-DTARGET_SIPU_ARCH=150
        --clangopt=-DSITE_TARGET_SIPU_ARCH=150
        --clangopt=-DMMA_DTE_SINGLE_FAMILY_DISPATCH=1
        --clangopt=-DMMA_DTE_DISPATCH_CASES_HEADER=.../dispatch_cases_mxint8_generated.hpp

6127  /usr/bin/c++ -shared -o .../test_bf16_mxint8_host/libtile_mma_dte_mxint8.so
        .../_SipuObj/tile_mma_dte_mxint8/kernel/inst_bf16_mxi8.su.o

6138  [ 75%] Linking CXX executable .../build/testcase/.../test_bf16_mxi8_host
6140  /usr/bin/c++ ... CMakeFiles/test_bf16_mxi8_host.dir/test_bf16_mxi8_host.cpp.o
        -o .../test_bf16_mxi8_host
        -L.../sicx_sdk/.../lib -L.../func_test/test_bf16_mxint8_host
        -ltile_mma_dte_mxint8 -lsipurt -lsipu
        /usr/lib/x86_64-linux-gnu/libgtest.a /usr/lib/x86_64-linux-gnu/libgtest_main.a
```

逐条对照 `readelf` / `nm` 的实测结果：

| 检查项 | `test_bf16_mxi8_host`（可执行） | `libtile_mma_dte_mxint8.so` |
| --- | --- | --- |
| `readelf -S` 有 `__sipu_fatbin`？ | **没有** | **有**，`[16]`，**4,245,288 字节** |
| 有 `.sipu_kernel_meta`？ | **没有** | **有**，`[15]`，179,670 字节 |
| `.text` 大小 | 258 KB | 366 KB（另有 `.rodata` 166 KB） |
| `.so` 总大小 | 542 KB | 5,398,912 字节（≈5.4 MB） |
| `mma_dte` 符号 | `nm -D --undefined-only` 里是 **`U`（未定义）**，共 2 个特化 | `nm --defined-only` 里有 **`W`（弱定义）**的实现 |

可执行文件的 `DT_NEEDED`：

```
NEEDED  libtile_mma_dte_mxint8.so
NEEDED  libsipurt.so.0
NEEDED  libstdc++.so.6
NEEDED  libm.so.6
NEEDED  libgcc_s.so.1
NEEDED  libc.so.6
RPATH   [.../envs/siinfer_dev/lib : .../sicx_sdk/<tag>/lib : .../test_bf16_mxint8_host]
```

**`NEEDED libtile_mma_dte_mxint8.so` 就是答案**：device code 通过这一个 `.so` 在**运行时**被 `libsipurt.so.0` 加载。

#### 64.4.3 那 `.so` 里到底有多少 device 实现？—— 4214 个，不是 2 个

这是本问最容易踩的坑。把 `.so` 的 fatbin 段抽出来数：

```bash
objcopy -O binary --only-section=__sipu_fatbin libtile_mma_dte_mxint8.so fatbin.bin   # 4,245,288 字节
strings -n 20 fatbin.bin | grep '^_Z' | grep 'kernel_tile_mma' | sort -u | wc -l
# → 4214
```
...
| 维度 | 分布 |
| --- | --- |
| 总量 | **4214** 个 `kernel_tile_mma_procon_v2_tiled_tensor_map<...>` 特化 |
| 按 tile 通路（`loadL2B_mma_r*`） | r8 = 1498，**r16 = 0**，r32 = 2716 |
| 按**输入** dtype | `4mxi8` = **4214**（全部），`mxf6e3m2` = 0，`mxf6e2m3` = 0 |
| 按**输出** dtype | `SiFpBaseILj8ELj7Et`（bf16）= 1498，`SiFpBaseILj5ELj10Et`（fp16）= 2716 |

**怎么读这张表**：

- **输入 dtype 只有 mxint8** —— 这正是 `MMA_DTE_TEST_DISPATCH_FAMILY=mxint8` 的作用：`testcase/func_test/CMakeMmaDteCommon.cmake:37-56` 从**目录名**（`test_bf16_mxint8_host` 命中 `mxint8|mxi8` 正则，`:47-48`）自动推断出 family，`:65-67` 据此选 `dispatch_cases_mxint8_generated.hpp`，`:176-177` 再把它作为 `-DMMA_DTE_SINGLE_FAMILY_DISPATCH=1 -DMMA_DTE_DISPATCH_CASES_HEADER=...` 传给 `scc`。**所以别的族（mxfp8 / mxfp6 / mx4 / non_mx）一个都没进来** —— 这一层"裁剪"是生效的。
- **但族内的 shape/schedule 组合一个都没裁** —— 1498 + 2716 对应 `kernel/detail/generated/dispatch_cases_mxint8_generated.hpp` 里那些 `GEN_CASES_*` 宏展开出的全部 case（该文件 38 个 `#define GEN_CASES`，每个再展开成几十个 2D 组合）。`kernel/mma_dte_tiled_tensor.hpp:88-90` 的 `get_best_solution()` 是**运行时**才选具体配置的，编译器无法在编译期把它折叠成唯一一个，所以**整族都留在二进制里**。
- r16 = 0 是因为 `.su:19` 虽然 `#include` 了 r16 的头，但没有任何 r16 的 `template void` 实例化，所以没生成 device kernel。

**结论（Q4 的完整回答）**：

> 既不是"整主库的 device code 编成一个大 `.so`"，也不是"只把用到的少数几个编成 `.o`"。
> 而是：**test 自己的一份 `.su`（只实例化 2 个 `mma_dte` 模板）→ `scc` 编成 1 个 `.o` → `/usr/bin/c++ -shared` 打成 1 个私有 `.so` → 可执行文件 `-l` 这个 `.so`**。
> **可执行文件里 0 字节 device code**（无 fatbin 段）；**那个 `.so` 里有 4214 个 device kernel** —— 输入 dtype 被裁剪到只剩 mxint8，但 mxint8 族内的全部 shape/schedule 组合都在。

device code 的注册是 host stub 干的：`.so` 的未定义符号里有 `__sipuRegisterFatBinary`、`__sipuRegisterFunction`、`sipuLaunchKernelExC`、`__sipuPushCallConfiguration`、`__sipuPopCallConfiguration`，这些都由 `libsipurt.so.0` 提供。

#### 64.4.4 生产库走的是另一条路（本 test 未使用）

作为对照，生产库 `CMakeLists.txt` 的做法是（`:322-340`）：

```cmake
322  foreach(TYPE IN LISTS OBJECT_LIB_TYPES)
324      set(COMPONENT_SHARED_LIB "${CMAKE_CURRENT_BINARY_DIR}/${MMA_DTE_LIB_NAME}_${TYPE}.so")
326      add_custom_command(
327          OUTPUT ${COMPONENT_SHARED_LIB}
328          COMMAND ${CMAKE_CXX_COMPILER} -shared
329              -Wl,-soname,${MMA_DTE_LIB_NAME}_${TYPE}.so
330              -o ${COMPONENT_SHARED_LIB}
331              ${OBJECT_FILE} ...
```

即 **每个 `inst_*.su`（每个 `TYPE`）各自编成一个 `.o`，再各自打成一个 `libtile_mma_dte_<TYPE>.so`**，最后用一个 `mma_dte_umbrella`（`:363`）把 40 个 DSO 通过生成的 trampoline 暴露出去。注释里给了原因（`:318-321`）：

> Linking every instantiation object into a single ELF overflows x86_64 PC-relative relocations. Build one private DSO per instantiation...

**所以哪怕在"整个主库"这条路里，也不是一个大 `.so`，而是 40 个小 `.so` 加一个 umbrella。**

`test_bf16_mxint8_host` 是否复用了生产库的 `.o`？**没有。** 复用机制在 `testcase/func_test/CMakeMmaDteCommon.cmake:209` 的 `mma_dte_get_main_object()`（它要求生产库的 `mma_dte_build_manifest.txt`、`mma_dte_build_inputs.stamp`、SDK 路径、arch、`MMA_DTE_ENABLE_SPLITK`、profile、`MMA_DTE_TMAP_LMUL` 全部对得上，还要过 `:302-313` 的逐文件时间戳检查）。全仓库只有 **4 个** testcase 调用它（`test_bf16_fp16_r32_host`、`test_bf16_fp16_fp32_r32_host`、`test_bf16_mxfp4_r32_host`、`test_int32_int8_r32_host`），**`test_bf16_mxint8_host/CMakeLists.txt` 里没有任何 `mma_dte_get_main_object` 或 `MMA_DTE_MAIN_BUILD_DIR` 引用**，所以它永远走 `:43` 的本地 `scc` 编译。

---

### 64.5 补充：`build.sh` vs `build-modify.sh`，以及和 74 个 testcase 的关系

用到的命令是 `BUILD_CXX=/usr/bin/c++ bash build-modify.sh --test`：

| | `build.sh` | `build-modify.sh` |
| --- | --- | --- |
| `BUILD_CXX` 支持 | 有（`:67-69`） | 有 |
| 构建目录 | `build/` | `build-cxx/`（`temp/cxx.log` 里可见） |
| 构建成功了吗 | 曾失败过：16/74 个 testcase 子项目在 `find_package(GTest REQUIRED)` 处 CMake configure 失败（见 64.5.1） | **成功**，`temp/cxx.log` 末尾 `[100%] Built target mma_dte_all_testcases`，501 个 `Built target` |

**为什么 `build-modify.sh` 能过而 `build.sh` 不能**：当时 gtest 没装，`build.sh` 挂在 `find_package(GTest REQUIRED)`（`testcase/func_test/CMakeMmaDteCommon.cmake:353`）。现在 `/usr/lib/x86_64-linux-gnu/libgtest.a` 与 `libgtest_main.a` 已经存在（`temp/cxx.log:6140` 的链接行里能看到），所以这条路通了。

另外 `build-modify.sh` 的 `--test` 只构建 testcase 部分，**不构建主库**（`build_library()` 没被调用），所以 `build/` 下根本没有 `libtile_mma_dte_<TYPE>.so`。

#### 64.5.1 顺带：`build.sh --all` 曾经挂在 gtest 上

之前 `cmake.log` 那次的失败与本例代码无关，是环境缺 gtest：

- 阶段 1（生产库）是成功的（`cmake.log:6-8`：`Built target mma_dte_build_manifest` / `mma_dte_umbrella` / `tile_mma_dte`）。
- 阶段 2 挂在 CMake configure，`find_package(GTest REQUIRED)` 只有 **3 处直接调用**，但覆盖全部 74 个子项目：
  - `testcase/func_test/CMakeMmaDteCommon.cmake:353` —— 在函数 `mma_dte_test_enable_gtest` 里（`:352` 定义）。**绝大多数 func_test 都走这个函数**，所以这一处就决定了几乎全部子项目的成败。
  - `testcase/autotune/CMakeLists.txt:23`
  - `testcase/func_test/test_validation_host/CMakeLists.txt:27`
  - （`testcase/func_test/splitK/common/SplitKTest.cmake:54` 是在函数 `add_mma_dte_splitk_test` 里**转调** `mma_dte_test_enable_gtest`，不是独立的 `find_package` 点。）
- 当时 `GTEST_LIBRARY` / `GTEST_INCLUDE_DIR` / `GTEST_MAIN_LIBRARY` / `GTest_DIR` 全是 `-NOTFOUND`，而 **`/usr/lib/x86_64-linux-gnu/libgtest.a` 现在已存在**，所以现在 `build.sh` 和 `build-modify.sh` 都能过。

> 这也解释了一个现象：`build/` 下只有 `testcase/`，没有主库产物 —— 因为构建失败发生在阶段 2 的 configure，阶段 1 的 `.o`/`.so` 产物在**那次**构建的其他目录里，而本次 `--test` 又完全不碰主库。

---

### 64.6 小结

- **`sipu::` 的那套 tensor API（`make_shape` / `make_layout` / `make_tensor` / `LayoutTag` / `Tensor` / `mm_mnk`）全部来自 SDK 的 `SiTe` 库**，不是 sikernel 仓库的代码。主文件是 `$SI_SDK_ROOT/include/SiTe/tensor/tensor.hpp`（`namespace sipu` 在 `:38-2765`）；`make_shape` 在 `:343`、`make_layout` 在 `:597`、`make_tensor` 在 `:2483`、`mm_mnk` 在 `:2679`。
- **`mma_dte` 的模板声明在 sikernel 的 `include/simma.h:179`，实现在 `kernel/mma_dte_tiled_tensor.hpp:193 → 165 → 151 → 54`，再按 `layoutA.tile_dim1` 路由到 `mma_r32_dte` / `mma_r16_dte` / `mma_r8_dte`（`kernel/detail/mma_dte_tiled_tensor_dispatch_entrypoints.hpp:1436 / 1321 / 1211`）**；**被编译的实例只有** `testcase/func_test/test_bf16_mxint8_host/kernel/inst_bf16_mxi8.su:24` 和 `:31` 那 2 个显式实例化。
- **device code 的链接方式（Q4）**：`inst_bf16_mxi8.su` →（`scc`）→ 1 个 `.o` →（`/usr/bin/c++ -shared`）→ `libtile_mma_dte_mxint8.so` →（`-ltile_mma_dte_mxint8`）→ 可执行文件。**可执行文件本身不含 device code**（实测无 `fatbin` 段，542 KB）；device code 在 `.so` 里（4.2 MB fatbin）。
- **`.so` 里的 device 实现是 4214 个，不是 2 个。** 输入 dtype 被 `MMA_DTE_TEST_DISPATCH_FAMILY=mxint8` 裁剪到只剩 mxint8 一族，但**族内的全部 shape/schedule 组合都保留**（r8 = 1498，r32 = 2716），因为具体配置是运行期由 `get_best_solution()`（`kernel/mma_dte_tiled_tensor.hpp:88`）选的。
- **生产库走的是完全不同的路子**：40 个 `inst_*.su` 各自一个 `.o`、各自一个 `.so`，再加一个 `mma_dte_umbrella`（`CMakeLists.txt:322-340`、`:363`）。本 test **不复用**生产对象 —— 全仓库只有 4 个 testcase 走 `mma_dte_get_main_object()`（`CMakeMmaDteCommon.cmake:209`）。
- **例子里的 `alpha=0.1f` / `beta=0.2f` 是无效参数** —— `kernel/mma_dte_tiled_tensor.hpp:69` 的 `// TODO: Use transa, transb, alpha, beta, lda, ldb, ldc in implementation` 说明它们尚未接入，这也是为什么 golden 用纯 `A×B`（`mm_mnk`）就能逐 bit 对上。

## 65. `test_bf16_mxint8_host/CMakeLists.txt` 逐行讲解：`scc_target_get_output`、`add_custom_target`、`mma_dte_test_enable_gtest` 各自做什么

**问题**（两个）：

1. `testcase/func_test/test_bf16_mxint8_host/CMakeLists.txt` 里，`scc_target_get_output(tile_mma_dte_mxint8 TILE_MMA_DTE_MXINT8_O)`、`add_custom_target`、`mma_dte_test_enable_gtest` 这三个分别有什么功能？
2. 对这份 `CMakeLists.txt` 逐行讲解（忽略注释）。

**先纠正一处环境信息**：这次给的日志 `temp/cxx.log.nodir` 里，`bash build-modify.sh --test` 实际解析到的 SDK 是 **`2609161442`**，不是提问里写的 `2609151958`。证据是 ``temp/cxx.log.nodir:18`` 与 ``:5781``：

```
+ '[' -z /share_data/sicx_sdk/release/2609161442 ']'          ← setup.sh 导出的 SI_SDK_ROOT
+ TESTCASE_SDK_TAG=2609161442
/share_data/sicx_sdk/release/2609161442/bin/scc -arch=sipu_150 -c -o .../inst_bf16_mxi8.su.o ...
```

`2609151958` 这个目录**确实存在**（我确认过），只是这次构建没用到它 —— 因为 `setup.sh` 走的是 `/share_data/sicx_sdk/release/latest` 这个**软链**，而 `latest` 一直在动：调查时是 `2609161948`，现在是 `2609171350`。不过好消息是 **`2609151958` 和 `2609161442` 的 `sccConfig.cmake` 完全相同**（md5 都是 `f41bc964706b28a5c27fc6c55618997a`），所以下面引用的行号对两个 SDK 都成立。

另外，这次 `cxx.log.nodir` 的构建**是成功的**：末尾 ``:10970`` 是 `[100%] Built target mma_dte_all_testcases`，全日志 497 个 `Built target`，`grep -c "Error\|error:"` 为 **0**。文件名里的 `nodir` 对应的是这次没有用 `-B .../build-cxx` 那种带独立构建目录的分支，而是直接用 `build/`（`temp/cxx.log` 那次的 `BUILD_DIR=build-cxx`，产物落在 `build-cxx/`）。
...
| 名字 | 定义位置 | 一句话作用 |
| --- | --- | --- |
| `scc_target_get_output` | `$SI_SDK_ROOT/share/cmake/scc/sccConfig.cmake:876` | 把 scc target 编出来的**产物路径**取回到一个调用者指定的变量里。OBJECT 库返回**目标文件列表**，其他类型返回**单个输出文件路径** |
| `add_custom_target` | CMake 内建命令（非本项目/非 SDK 定义） | 建一个**永远不产生文件、只做依赖聚合**的 phony target |
| `mma_dte_test_enable_gtest` | `testcase/func_test/CMakeMmaDteCommon.cmake:352` | 给一个测试可执行文件**接上 gtest**：找包、链接 `GTest::gtest`/`GTest::gtest_main`、并把它的 test case 注册进 CTest |

---

### 65.1 `scc_target_get_output(tile_mma_dte_mxint8 TILE_MMA_DTE_MXINT8_O)` 有什么功能

定义在 `$SI_SDK_ROOT/share/cmake/scc/sccConfig.cmake:876`，函数名 `scc_target_get_output`，签名 `(TARGET OUTPUT_VARIABLE)` —— 参数 1 是目标名，参数 2 是**接收结果的变量名**（调用者的变量，`PARENT_SCOPE` 回写）。

它的逻辑是**按 target 的库类型分两种返回**：

```cmake
876  function(scc_target_get_output TARGET OUTPUT_VARIABLE)
888      get_target_property(LIB_TYPE ${TARGET} SIPU_LIBRARY_TYPE)
891      if(LIB_TYPE STREQUAL "OBJECT")
892          get_target_property(OBJ_FILES ${TARGET} SIPU_OBJECT_FILES)
899          set(${OUTPUT_VARIABLE} ${OBJ_FILES} PARENT_SCOPE)     ← OBJECT 库：返回 .o 列表
905          get_target_property(OUTPUT_FILE ${TARGET} SIPU_OUTPUT_FILE)
        set(${OUTPUT_VARIABLE} ${OUTPUT_FILE} PARENT_SCOPE)       ← 其他：返回单个产物路径
```

它的存在意义是：**`scc_add_library` 建的 target 不是标准 CMake target**，而是"自定义命令 + phony target"的组合（见 65.1.1），所以 `$<TARGET_FILE:...>` 这类标准生成式**对它不生效**，必须通过这个 helper 去读私有属性。

#### 65.1.1 为什么需要它 —— `scc_add_library` 到底建了什么

`testcase/func_test/test_bf16_mxint8_host/CMakeLists.txt:43` 调用的 `scc_add_library(tile_mma_dte_mxint8 OBJECT ${TEST_INST_SRC})` 定义在 `sccConfig.cmake:311`。它做三件事：

1. **为每个源文件建一条 `add_custom_command`**（`:426-427` 算路径，`:431` 建命令）：
   ```cmake
   426  set(OBJECT_FILE "${CMAKE_CURRENT_BINARY_DIR}/_SipuObj/${TARGET}/${REL_SOURCE_SAFE}.o")
   ...
        add_custom_command(OUTPUT ${OBJECT_FILE} COMMAND ${SCC_EXECUTABLE} ${COMPILE_OPTIONS} -o ${OBJECT_FILE} ...)
   ```
   这正好解释了日志里那个路径形状 —— ``cxx.log.nodir:5780`` 的
   ```
   [ 25%] Compiling .../kernel/inst_bf16_mxi8.su -> .../_SipuObj/tile_mma_dte_mxint8/kernel/inst_bf16_mxi8.su.o
   ```
   `$TARGET` = `tile_mma_dte_mxint8`，`${REL_SOURCE_SAFE}` = `kernel/inst_bf16_mxi8.su`（相对源目录的路径）。

2. **OBJECT 类型时，只建一个 phony target 挂在那些 `.o` 上**（`:449`）：
   ```cmake
   449  add_custom_target(${TARGET} ALL DEPENDS ${OBJECT_FILES})
   ```
   **注意：这里没有 link 步骤。** `.su` 只是被编成 `.o` 就结束了。

3. **把路径和类型存进私有 target 属性**（`:491-498`）：
   ```cmake
   494  SIPU_SOURCE_FILES "${ABSOLUTE_SOURCE_FILES}"
   495  SIPU_OBJECT_FILES "${OBJECT_FILES}"
   496  SIPU_COMPILER_OPTIONS "${COMPILE_OPTIONS}"
   497  SIPU_LIBRARY_TYPE "${LIBRARY_TYPE}"
   ```
   **`scc_target_get_output` 读的就是 `:495`（`SIPU_OBJECT_FILES`）和 `:497`（`SIPU_LIBRARY_TYPE`）这两个属性。**

所以 `scc_target_get_output(tile_mma_dte_mxint8 TILE_MMA_DTE_MXINT8_O)` 在 `CMakeLists.txt:50` 这一行的实际效果是：

```
TILE_MMA_DTE_MXINT8_O  ←  .../_SipuObj/tile_mma_dte_mxint8/kernel/inst_bf16_mxi8.su.o
```

也就是**把"那个由 scc 编出来的 .o 在哪"这件事取出来**，给下面 `:52` 的 `add_custom_command` 用。变量名 `TILE_MMA_DTE_MXINT8_O` 是调用者自己起的，`_O` 是 object 的意思。

> 一句话：**它是 scc 私有 target 体系的"取产物路径"接口**，因为标准 CMake 的 `$<TARGET_FILE:>` 在 scc target 上不可用。

---

### 65.2 `add_custom_target` 有什么功能

这是 **CMake 内建命令**，不是本仓库也不是 SDK 定义的。它的语义和 `add_custom_command(OUTPUT ...)` 正好互补：

| | `add_custom_command(OUTPUT ...)` | `add_custom_target(...)` |
| --- | --- | --- |
| 产出 | **真实文件**，可以被依赖、可以增量判断 | **不产出文件**，永远是"脏"的 |
| 能否被 `add_dependencies` 依赖 | 不能直接依赖（要依赖它的输出文件） | **能** |
| 每次构建 | 产物新则跳过 | **永远执行** |

在这份 `CMakeLists.txt` 里它出现了**两次**，用法不同：

**（a）`:60-62` —— 建一个"确保 `.so` 被生成"的 phony target：**

```cmake
60  add_custom_target(tile_mma_dte_mxint8_so ALL
61      DEPENDS ${CMAKE_CURRENT_BINARY_DIR}/libtile_mma_dte_mxint8.so
62  )
```

因为 `:52-58` 的 `add_custom_command` 产出的是一个**文件**，而 `add_dependencies` 只能依赖 target，不能依赖文件。所以这里用一个 `add_custom_target` **把那个文件包成一个 target**，好让 `:79` 能写：

```cmake
79  add_dependencies(test_bf16_mxi8_host tile_mma_dte_mxint8_so)
```

`ALL` 关键字表示这个 target 属于默认构建目标（`make` 不带参数也会建它）。

> 顺带说明：其实 `:74-78` 的 `target_link_libraries` 里已经写了 `${CMAKE_CURRENT_BINARY_DIR}/libtile_mma_dte_mxint8.so`，理论上 CMake 能通过文件名依赖自动推导出构建顺序。`add_dependencies` 这里是**显式的双保险**，保证链接前 `.so` 一定已经存在。

**（b）SDK 内部** —— `scc_add_library` 自己也用 `add_custom_target` 建 target（OBJECT 是 `sccConfig.cmake:449`，SHARED/STATIC 是 `:486`）。

**为什么这个文件里 `add_custom_target` 只出现一次（不把 `:67` 的 `add_executable` 算进来的话）**：因为 `test_bf16_mxi8_host` 本身是标准 `add_executable`（`:67`），不需要再包一层。

---

### 65.3 `mma_dte_test_enable_gtest(test_bf16_mxi8_host)` 有什么功能

定义在 `testcase/func_test/CMakeMmaDteCommon.cmake:352`，函数名 `mma_dte_test_enable_gtest`，只接一个参数 `target_name`。全文 11 行：

```cmake
352  function(mma_dte_test_enable_gtest target_name)
353      find_package(GTest REQUIRED)
354      include(GoogleTest)
355      target_link_libraries(${target_name} PRIVATE
356          GTest::gtest
357          GTest::gtest_main
358      )
359      gtest_discover_tests(${target_name}
360          DISCOVERY_MODE PRE_TEST
361      )
362  endfunction()
```
...
**（1）`:353` `find_package(GTest REQUIRED)`** —— 找系统 gtest。这是**整个 testcase 阶段最容易挂的一步**：上一版分析里 `build.sh --all` 就是挂在这里（`GTEST_LIBRARY` / `GTEST_INCLUDE_DIR` / `GTEST_MAIN_LIBRARY` 全 `-NOTFOUND`）。CMake 4.2 的 `FindGTest.cmake:421` 是 `find_package_handle_standard_args(GTest DEFAULT_MSG GTEST_LIBRARY GTEST_INCLUDE_DIR GTEST_MAIN_LIBRARY)` —— 所以缺的是**三个变量**，不是"找不到头文件"一条。这次能过，是因为 `/usr/lib/x86_64-linux-gnu/libgtest.a` 和 `libgtest_main.a` 已存在，日志 ``:5945`` 的链接行末尾能看到它们被展开（`... -ltile_mma_dte_mxint8 -lsipurt -lsipu /usr/lib/x86_64-linux-gnu/libgtest.a /usr/lib/x86_64-linux-gnu/libgtest_main.a /usr/lib/x86_64-linux-gnu/libgtest.a`）。

**（2）`:355-358` 链接两个 imported target** —— `GTest::gtest` 和 `GTest::gtest_main` 是 `FindGTest.cmake` 提供的 imported target。**两个都要链**：`gtest` 是框架本体，`gtest_main` 提供那个 `int main()`（否则 `TEST_F` 宏写出来的测试没有入口，链接会报 `undefined reference to main`）。

**（3）`:359-361` `gtest_discover_tests(... DISCOVERY_MODE PRE_TEST)`** —— 把可执行文件里的 test 列表**枚举出来注册进 CTest**，这样 `ctest` 能跑、也能 `ctest -R` 过滤单个 case。`DISCOVERY_MODE` 有两种：

| 模式 | 何时枚举 | 代价 |
| --- | --- | --- |
| `POST_BUILD`（CMake 默认） | 每次链接完**立刻**跑一次 `--gtest_list_tests` | 每次构建都多跑一次程序 |
| `PRE_TEST`（本仓库选它） | 推迟到 **`ctest` 真跑之前**才枚举 | 构建时不额外执行 |

`include(GoogleTest)` 提供 `gtest_discover_tests`，模块在 `.../cmake-4.2/Modules/GoogleTest.cmake`；`DISCOVERY_MODE` 的默认值逻辑在 `GoogleTest.cmake:585-589`：

```cmake
585  if(NOT arg_DISCOVERY_MODE)
586      if(NOT CMAKE_GTEST_DISCOVER_TESTS_DISCOVERY_MODE)
587          set(CMAKE_GTEST_DISCOVER_TESTS_DISCOVERY_MODE "POST_BUILD")
589      set(arg_DISCOVERY_MODE ${CMAKE_GTEST_DISCOVER_TESTS_DISCOVERY_MODE})
```

**为什么非要显式写 `PRE_TEST`**：因为这是个交叉编译/仿真环境（要起 cmodel 才能运行），`POST_BUILD` 会在**每次构建时都尝试执行目标程序**，在这个环境里既慢又可能直接失败。聚合层的 `testcase/CMakeLists.txt:87` 也把 `-DCMAKE_GTEST_DISCOVER_TESTS_DISCOVERY_MODE:STRING=PRE_TEST` 透传给每个子项目（日志 ``:5740`` 有这条实参），这里是第二层保险。

> 这个函数被 **3 类地方**引用：`testcase/func_test/CMakeMmaDteCommon.cmake:353` 所在的定义点被绝大多数 func_test 直接调用；`testcase/func_test/splitK/common/SplitKTest.cmake:54` 在 `add_mma_dte_splitk_test` 里转调；`testcase/autotune/CMakeLists.txt:23` 和 `testcase/func_test/test_validation_host/CMakeLists.txt:27` 则各自直接写 `find_package(GTest REQUIRED)`，没走这个函数。

---

### 65.4 `CMakeLists.txt` 逐行讲解（忽略注释）

文件路径：`testcase/func_test/test_bf16_mxint8_host/CMakeLists.txt`（81 行）。

#### 65.4.1 头三行：最低版本与标准

| 行 | 代码 | 说明 |
| --- | --- | --- |
| `2` | `cmake_minimum_required(VERSION 3.27)` | 最低 CMake 3.27。注：本机实际用的是 `cmake 4.2.0-rc2`。3.27 这个下限有实际后果 —— `testcase/CMakeLists.txt` 里判断 `BUILD_JOB_SERVER_AWARE`（3.28 才有）时就在兼容这个下限 |
| `3` | `set(CMAKE_CXX_STANDARD 20)` | C++20。这个必须的：`:33-37` 那些 `constexpr auto kLayoutA_R8 = ...` 被当作**非类型模板参数**传给 `run_case<...>`（`test_bf16_mxi8_host.cpp:40-42`），依赖 C++20 的 class-type NTTP |
| `6-10` | `project(mma_dte_mxint8_test VERSION 1.0.0 DESCRIPTION "..." LANGUAGES CXX)` | 工程名 `mma_dte_mxint8_test`。**注意这是"名字"，不是任何产物名** —— 下面所有 target 都是单独命名的。`LANGUAGES CXX` 只声明 C++，所以这个子项目里没有 C 编译器被启用 |

#### 65.4.2 引入公共配置

| 行 | 代码 | 说明 |
| --- | --- | --- |
| `12` | `find_package(scc REQUIRED)` | 找 SDK 的 scc 工具链包（`$SI_SDK_ROOT/share/cmake/scc/sccConfig.cmake`）。**这是 `scc_add_library` / `scc_target_compile_options` / `scc_target_include_directories` / `scc_target_get_output` 四个函数的来源**。找不到就直接 configure 失败 |
| `13` | `include(${CMAKE_CURRENT_LIST_DIR}/../CMakeMmaDteCommon.cmake)` | 把 74 个子项目共用的配置模块拉进来（`testcase/func_test/CMakeMmaDteCommon.cmake`，363 行）。它带来的东西见 65.4.2.1 |

**65.4.2.1 `CMakeMmaDteCommon.cmake` 被 include 时**（在 include 那一刻就执行的顶层语句）**立即做的事**：

| 行 | 代码 | 说明 |
| --- | --- | --- |
| `1` | `include("${CMAKE_CURRENT_LIST_DIR}/../../cmake/MmaDteConfig.cmake")` | 再拉一层公共 cmake 模块（`cmake/MmaDteConfig.cmake`） |
| `3` | `mma_dte_find_root("${CMAKE_CURRENT_LIST_DIR}" MMA_DTE_ROOT)` | **从当前目录往上找仓库根**。判据是"某目录下同时存在 `include/simma.h` 和 `kernel/mma_dte_tiled_tensor.h`"（`cmake/MmaDteConfig.cmake:4-5`）。找到的是 `/share/users/like/package/sikernel/mma_dte_tile_tensor` |
| `4-5` | `set(MMA_DTE_DIR ...)` / `set(MMA_DTE_KERNEL_DIR ...)` | 派生两个路径变量 |
| `6` | `mma_dte_configure_sipu_arch()` | 决定目标架构（150/160/170）。`cmake/MmaDteConfig.cmake:96` 定义，探测顺序在 `mma_dte_detect_sipu_arch`（`:53`）里，优先级是 `TARGET_SIPU_ARCH` → 环境变量 `SIPU_ARCH` → `SI_CMODEL_HW_ARCH` → `SIKERNEL_SIPU_ARCH_VERSION` |
| `69` | `mma_dte_common_include_directories("${MMA_DTE_ROOT}" MMA_DTE_COMMON_INCLUDE_DIRECTORIES)` | **生成 include 路径列表**：SDK 的 include（`mma_dte_collect_sdk_include_directories`，`cmake/MmaDteConfig.cmake:19`）+ 仓库的 `include` / `kernel` / `kernel/util`（`cmake/MmaDteConfig.cmake:41-50`） |
| `74` | `enable_testing()` | 打开本子项目的 CTest 支持。**`gtest_discover_tests` 必须在它之后调用**，否则 `:359` 注册的测试不生效 |
| `76` | `mma_dte_add_target_arch_compile_definition(_SIKERNEL_TARGET_ARCH_COMPILE_DEFINITION)` | 生成形如 `-DTARGET_SIPU_ARCH=150` 的定义（`cmake/MmaDteConfig.cmake:173`），`:174` 再由 `mma_dte_test_setup_compile_options` 拼进编译选项 |
| `80-83` | `add_compile_definitions(TARGET_SIPU_ARCH=${...} SITE_TARGET_SIPU_ARCH=${...})` | 把 arch 宏也加给 **host** 编译（上面那个只作用于 scc 的 device 编译）。注释里说明原因：SiTe 在 host-only 的 `make_tensor`/`make_layout` 序列化路径里会用到这个宏 |
| `86-87` | `add_compile_options($<$<COMPILE_LANGUAGE:CXX>:-include> $<$<COMPILE_LANGUAGE:CXX>:iostream>)` | **强制预包含 `<iostream>`**。注释交代了原因：某些 SDK 版本的头文件里用了 `std::cerr` 却没 include。用生成式限定了只对 C++ 生效 |

`:89-162` 是 **dispatch case 头文件的代码生成**：检查 `kernel/detail/generated/` 下 9 个头文件（`:90-102`）是否存在、是否比 3 个 Python 脚本（`:117-120`）旧、以及 profile 是否变化（`:135-143`，靠读 `dispatch_config_generated.hpp` 前 512 字节里的 `// Scope: delivery profile=xxx` 标记），任一不满足就调 `python3 tools/gen_dispatch_cases.py` 重新生成（`:146-158`）。这段**在本文件里没有显式调用**，是 include 时的副作用。

`:37-68` 是本项目最关键的配置之一 —— **dispatch family 自动推断**：`MMA_DTE_TEST_DISPATCH_FAMILY` 默认 `auto`（`:37`），`:42-53` 拿**当前源目录路径**去匹配，`:47-48` 命中 `mxint8|mxi8` 就设成 `mxint8`。`test_bf16_mxint8_host` 这个目录名正好命中。`:63-67` 据此定下要用的 cases 头 `kernel/detail/generated/dispatch_cases_mxint8_generated.hpp`，`:68` 打印一行 `-- func_test dispatch family: mxint8`。**这就是为什么这个 test 只编 mxint8 一族的 device code。**

#### 65.4.3 本文件自己的配置（`:15-36`）

| 行 | 代码 | 说明 |
| --- | --- | --- |
| `16` | `set(FUNC_TEST_KERNEL_DIR "${CMAKE_CURRENT_SOURCE_DIR}/kernel")` | 指向本目录的 `kernel/`，里面就是那个 `inst_bf16_mxi8.su` |
| `19` | `option(MMA_DTE_GENERIC_TAIL "..." OFF)` | 布尔开关，默认 OFF。ON 时走"通用尾处理"分支而不是逐 shape 特化。**注意这个 option 在本文件里只影响 `:33-35` 的选项拼接**，真正的值是通过 `testcase/CMakeLists.txt` 的缓存变量白名单传下来的 |
| `20-21` | `set(TEST_GRID_DIM "16" CACHE STRING "...")` | 网格维度覆盖。类型是 `CACHE STRING`，所以**可以被父项目用 `-DTEST_GRID_DIM=...` 覆盖**（本目录的 `build.sh:20` 有 `${TEST_GRID_DIM:+-DTEST_GRID_DIM=${TEST_GRID_DIM}}`） |
| `22-23` | `set(TEST_ENABLE_DEBUG_PRINT OFF CACHE BOOL "...")` | 同上，调试打印开关 |
| `24-29` | `mma_dte_test_setup_compile_options(COMMON_COMPILE_OPTIONS -DSIRT_DEVICE_USE_PRINTF -fPIC -DDTE_DEV_MODE -DMMA_DTE_TEST_GRID_DIM=${TEST_GRID_DIM})` | **拼装 device 侧编译选项**。函数定义在 `CMakeMmaDteCommon.cmake:173`；它会把传进来的这些选项**再追加**上 `-DMMA_DTE_SINGLE_FAMILY_DISPATCH=1` 和 `-DMMA_DTE_DISPATCH_CASES_HEADER=<family 头>`（`:176-177`），以及可选的 `-DMMA_DTE_ENABLE_SPLITK`（`:178-180`）和 `-DMMA_DTE_TMAP_LMUL`（`:181-192`）。**这就是日志 ``:5781`` 里那串 `--clangopt=` 的来源。** 四个选项的作用：`-DSIRT_DEVICE_USE_PRINTF` 开设备端 printf；`-fPIC` 位置无关代码（后面要打进 `.so`）；`-DDTE_DEV_MODE` 放宽 `kernel/mma_dte_tiled_tensor.hpp:64` 的 `#ifndef DTE_DEV_MODE` 保护（production 路径的 layout contract 静态断言在测试模式下被跳过）；`-DMMA_DTE_TEST_GRID_DIM=16` 固定 grid 试探值 |
| `30-32` | `if(TEST_ENABLE_DEBUG_PRINT) list(APPEND ... -DKERNEL_DBG) endif()` | 开关打开才追加 `-DKERNEL_DBG`（对应 `kernel/sipu_kernel_debug.h`） |
| `33-35` | `if(MMA_DTE_GENERIC_TAIL) list(APPEND ... -DMMA_DTE_GENERIC_TAIL) endif()` | 同上，追加宏 |
| `36` | `mma_dte_test_setup_include_dirs(COMMON_INCLUDE_DIRECTORIES)` | 取 include 列表。定义在 `CMakeMmaDteCommon.cmake:164`，本质是把 `:69` 生成的 `MMA_DTE_COMMON_INCLUDE_DIRECTORIES` 拷一份给它，并去掉重复项 |

#### 65.4.4 编 `.su`（`:38-47`）

| 行 | 代码 | 说明 |
| --- | --- | --- |
| `41` | `set(TEST_INST_SRC "${FUNC_TEST_KERNEL_DIR}/inst_bf16_mxi8.su" CACHE STRING "...")` | 指向本 test 的 device 实例化源。**`CACHE STRING` 意味着可被外部覆盖** —— 注释里说得很清楚，用途是"批量构建时只编当前用例需要的实例化" |
| `43-45` | `scc_add_library(tile_mma_dte_mxint8 OBJECT ${TEST_INST_SRC})` | **用 scc 把 `.su` 编成 OBJECT 库**。第 2 个参数 `OBJECT` 决定库类型（`sccConfig.cmake:318` 的 `set(options SHARED STATIC OBJECT)` 解析它），`:345-346` 起据此定 `LIBRARY_TYPE` / `LIBRARY_EXTENSION`。产生的 target 名就是 `tile_mma_dte_mxint8` |
| `46` | `scc_target_compile_options(tile_mma_dte_mxint8 ${COMMON_COMPILE_OPTIONS})` | 把 `:24` 拼好的选项挂到 target 上。定义在 `sccConfig.cmake:705`。**注意参数没写 `PRIVATE`/`PUBLIC`** —— 于是落到 `:719` 的 `UNPARSED_ARGUMENTS`，被当成默认选项处理 |
| `47` | `scc_target_include_directories(tile_mma_dte_mxint8 ${COMMON_INCLUDE_DIRECTORIES})` | 同理挂 include 路径。定义在 `sccConfig.cmake:512`。**它把相对路径转成绝对路径**（`:547-551`）再存进私有属性 `SIPU_PRIVATE_INCLUDE_DIRECTORIES`（`:553-555`） |

#### 65.4.5 取产物路径、打 `.so`（`:49-62`）

| 行 | 代码 | 说明 |
| --- | --- | --- |
| `50` | `scc_target_get_output(tile_mma_dte_mxint8 TILE_MMA_DTE_MXINT8_O)` | **见 65.1**。结果是那个 `.su.o` 的绝对路径 |
| `52-58` | `add_custom_command(OUTPUT .../libtile_mma_dte_mxint8.so COMMAND ${CMAKE_CXX_COMPILER} -shared -o ... ${TILE_MMA_DTE_MXINT8_O} DEPENDS tile_mma_dte_mxint8 ${TILE_MMA_DTE_MXINT8_O} COMMENT "Linking libtile_mma_dte_mxint8.so")` | **关键的一步**：用**普通 C++ 编译器**（`/usr/bin/c++`，不是 scc）把 scc 编出的那个 `.o` 打成 `.so`。`DEPENDS` 同时列了 **target 名**和**具体文件**，双保险。这条命令也在日志里：``cxx.log.nodir:5852`` |
| `60-62` | `add_custom_target(tile_mma_dte_mxint8_so ALL DEPENDS .../libtile_mma_dte_mxint8.so)` | **见 65.2（a）**：把"文件"包成"target"，好让 `:79` 的 `add_dependencies` 能用 |

> **`-shared` 能成功的原因**：`.su.o` 里已经含了 device 侧的 `fatbin` 段和 host 侧的 stub（`__sipuRegisterFatBinary` 等未定义符号，等运行期由 `libsipurt.so` 提供），所以这一步只是普通的 ELF 打包，不需要 device linker。

#### 65.4.6 建可执行文件（`:64-81`）

| 行 | 代码 | 说明 |
| --- | --- | --- |
| `66` | `set(TEST_HOST_SRC "${CMAKE_CURRENT_SOURCE_DIR}/test_bf16_mxi8_host.cpp" CACHE STRING "...")` | 宿主源码路径，同样可被外部覆盖（注释说用于并行构建） |
| `67-69` | `add_executable(test_bf16_mxi8_host ${TEST_HOST_SRC})` | **标准 CMake 可执行文件**，不是 scc target —— 因为 `.cpp` 是纯 host 代码，由 `/usr/bin/c++` 编译。**产物名 `test_bf16_mxi8_host` 就去掉了 `.cpp` 里的下划线**（源文件叫 `test_bf16_mxi8_host.cpp`，目标名一致） |
| `70-73` | `target_include_directories(test_bf16_mxi8_host PRIVATE ${COMMON_INCLUDE_DIRECTORIES} ${MMA_DTE_ROOT})` | host 侧也要 include 路径。这里**额外加了 `${MMA_DTE_ROOT}`**（仓库根），因为 `test_bf16_mxi8_host.cpp:29` 写的是 `#include "simma.h"`，而 `simma.h` 在 `include/` 下，靠 `COMMON_INCLUDE_DIRECTORIES` 里的 `include` 项就够；加根目录是为了让形如 `kernel/...` 的深路径包含也能写 |
| `74-78` | `target_link_libraries(test_bf16_mxi8_host PRIVATE ${CMAKE_CURRENT_BINARY_DIR}/libtile_mma_dte_mxint8.so sipurt sipu)` | 链三样：本 test 私有 `.so`（**用完整路径而非 `-l`，CMake 会自动转成 `-l` 并补 `-L`**）、SDK 的运行时库 `libsipurt.so`（提供 device 加载与 `LaunchKernel`）、`libsipu.so`（提供 `siTensorMapEncode*` 等）。**注意 `PRIVATE`**：这些依赖不会传递给依赖它的目标 |
| `79` | `add_dependencies(test_bf16_mxi8_host tile_mma_dte_mxint8_so)` | **见 65.2（a）**：保证链 `.so` 之前它已被构建 |
| `81` | `mma_dte_test_enable_gtest(test_bf16_mxi8_host)` | **见 65.3**。放在最后，因为它依赖 `:67` 的 target 已经存在 |

#### 65.4.7 这个子项目的完整产物链

```
testcase/.../kernel/inst_bf16_mxi8.su
   │  (:43 scc_add_library OBJECT)  → (:46 编译选项) → (:47 include)
   │  实际命令见 cxx.log.nodir:5781
   ▼
_SipuObj/tile_mma_dte_mxint8/kernel/inst_bf16_mxi8.su.o      ← (:50 取路径)
   │  (:52 add_custom_command, ${CMAKE_CXX_COMPILER} -shared)  cxx.log.nodir:5852-5853
   ▼
libtile_mma_dte_mxint8.so                                              ← (:60 包成 target)
   │  (:74 target_link_libraries) + (:79 add_dependencies)
   ▼
test_bf16_mxi8_host  ←──────────────────  test_bf16_mxi8_host.cpp (:67 add_executable)
   │  (:81 mma_dte_test_enable_gtest)
   ▼
注册进 CTest (PRE_TEST 发现)
```
...
- **`scc_target_get_output(tile_mma_dte_mxint8 TILE_MMA_DTE_MXINT8_O)`**（`CMakeLists.txt:50`）是 scc 私有 target 体系的"取产物路径"接口（定义在 `sccConfig.cmake:876`）。因为 `scc_add_library` 建的不是标准 CMake target 而是"`add_custom_command` + `add_custom_target`"的组合（`sccConfig.cmake:431-449`），所以 `$<TARGET_FILE:>` 用不了。它是 **OBJECT 库就返回 `.o` 列表**，其他类型返回单个输出文件路径；返回值通过 `PARENT_SCOPE` 写回调用者指定的变量。这里取出的就是 `.../_SipuObj/tile_mma_dte_mxint8/kernel/inst_bf16_mxi8.su.o`。
- **`add_custom_target`** 是 CMake 内建命令，用来建**不产出文件、只聚合依赖**的 phony target。本文件里用在 `:60-62`：因为 `add_dependencies` 只能依赖 target 而不能依赖文件，所以用它把 `add_custom_command` 产出的 `.so` **包一层**，供 `:79` 使用。它**每次构建都算脏**，但这里无所谓 —— 它本身不执行任何命令，只是把依赖传递下去。
...
## 68. 在 simo 中通过 `torch.ops.simo.mma_dte` 调用 mma_dte MX 矩阵乘法的方案（AOT）

> **结论先行（四个直接答案）**
>
> 1. **simo 需要增加 CMakeLists.txt 吗？——需要，但只需要一个很小的 CMakeLists.txt，且不是为了让 simo 自己的 host 代码走 CMake，而是为了让 `.su` 设备代码能被 scc 正确编译。** 如果采用本文推荐的「复用 torch 已编好的 kernel」方案，则**连一个 `.su` 都不需要写**，但仍然需要 CMake 或等价的构建步骤来把 host 代码链接到 `libtorch_sipu.so`。
> 2. **需要 tvm-ffi 吗？——不需要。** 全链路（torch、torch_sipu、siinfer、hpc-ops、mma_dte_tile_tensor）没有任何一处使用 tvm-ffi。
> 3. **需要 pybind11 吗？——不需要显式引入。** `torch.utils.cpp_extension` 已经自带 pybind11（在 `torch/include/pybind11/`），simo 现有的 `simo/csrc/torch_bindings.cpp:1` 就是直接 `#include <pybind11/pybind11.h>` 而 `pyproject.toml` 里并没有声明 pybind11 依赖。
> 4. **最小依赖方案：复用 `libtorch_sipu.so` 里已经编好的 MX mma_dte kernel，simo 侧只写 host C++。** 我在本机验证了：torch 的 wheel **同时导出了头文件和全部 MX 叶子符号**（含 `_fp32` 变体），simo 可以零设备代码编译地调用它们。

下面分三部分：先回答你问的关于 torch_sipu 的三个问题（68.1–68.3），再给方案设计（68.4–68.7），最后是依赖清单与分阶段实施建议（68.8–68.9）。

---

### 68.1 问题一：torch_sipu 的 .so 会显式依赖 `libtile_mma_dte.so` 吗？

**不会。** 我用 `readelf -d` 实测了 wheel 里所有 `.so`：

`libtorch_sipu.so`（481 MB，位于 `site-packages/torch_sipu/lib/`）的 `DT_NEEDED` 完整列表是：

```
libtorch.so, libtorch_cpu.so, libtorch_python.so,
libsipu_kernels.so, libsiblas.so.1, libsideepgemm.so,
libsideepgemm_mntail.so, libsidnn.so.0, libsisolver.so,
libsipu.so, libsipurt.so, libsipti.so,
libc10.so, libstdc++.so.6, libm.so.6, libgcc_s.so.1, libc.so.6, ld-linux-x86-64.so.2
```

**其中没有任何 `libtile_mma_dte*`。** 它的 RUNPATH 是 `$ORIGIN:$ORIGIN/150`。

更关键的是，我进一步确认了 torch **根本不需要**这个库——`nm -D` 显示 `libtorch_sipu.so` 对 `libtile_mma_dte` 的**未定义引用数为 0**，而它**自己就定义了** `mx_mma_dte_tiled_tensor`：

```
0000000002785be0 T _ZN2at6native23mx_mma_dte_tiled_tensorENS0_14MxMmaDteConfigES1_N3c1010ScalarTypeERKNS_6TensorES6_RS4_
```

也就是说，**MX 的 mma_dte 实现是被静态编进 `libtorch_sipu.so` 自己的**（共 231 个 `mx_mma_dte` 相关符号、105 个 `run_mx_mma_dte_tiled_tensor` 模板实例化）。

那 `lib/150/` 下的那 6 个 `libtile_mma_dte_*.so` 是干什么的？我追了一下依赖链：

- `lib/150/` 里只有 `bf16`/`fp16` 两种，**没有 MX 的**；
- 唯一 `DT_NEEDED` 它们的是 `libsiblas_gemm_dte_external_adapter_150.so`；
- 而 `libtorch_sipu.so` 只 `NEEDED libsiblas.so.1`，**并不直接需要那个 adapter**。

所以那 6 个 DSO 属于 siBLAS 的非 MX（bf16/fp16）GEMM 路径，**与你要的 MX matmul 完全无关**。

---

### 68.2 问题二：依赖的是 `libtile_mma_dte.so` 主库还是 component DSO？

**两个都不依赖。** 这里需要先纠正一个前提：`answer_codex_v4.md` §13 描述的「umbrella + component DSO」结构，是 **`mma_dte_tile_tensor` 这个独立 code base 自己的 release 产物形态**，不是 torch 的形态。

- 在 `mma_dte_tile_tensor` 自己的 release 包里（`/share/users/like/package/oplib/mma_dte_tile_tensor/release/mma_dte_tiled_tensor-1.0.0/lib/`），确实有 `libtile_mma_dte.so`（853 KB 的 dlopen trampoline）加 40 个 component DSO（`libtile_mma_dte_mxint8_r32_1x4.so` 等，最大 36 MB）。umbrella 通过 `dladdr()` + `dlopen()` 按完整 C++ mangled 符号名转发，实现在 `tools/generate_mma_dte_umbrella.py:57-98` 的 `__mma_dte_resolve_component_symbol()`。
- 但 **torch 没有采用这个形态**。torch 走的是 **header-only**（见 68.3），把 mma_dte 的**头文件**编进自己的 `.su` 编译单元，产出的 device fatbin 直接落在 `libtorch_sipu.so` 里。

所以对你的方案来说，`libtile_mma_dte.so` 和它的 component DSO **都不需要**——这是一个重要的简化。

---

### 68.3 问题三：torch 集成 mma_dte_tile_tensor 用的是方案 a 还是方案 b？

**是方案 a：torch 用自己的构建系统编译 mma_dte_tile_tensor 的 C++ 代码，完全无视其 CMakeLists.txt。**

证据链（三处独立证据互相印证）：

**证据一 —— torch 明确声明「header-only」。** `torch_sipu/cmake/public/mma_dte.cmake` 是 torch 接入 mma_dte 的入口（由 `cmake/Dependencies.cmake:32-35` 在 `USE_MMA_DTE_TILE_TENSOR` 打开时 include）。该文件末尾 `:101-104`：

```cmake
set(TORCH_SIPU_MMA_DTE_LIBRARY
    ""
    CACHE FILEPATH "mma_dte_tile_tensor is used header-only" FORCE)
set(TORCH_SIPU_MMA_DTE_SONAME_LIBRARY
    ""
    CACHE FILEPATH "mma_dte_tile_tensor is used header-only" FORCE)

set(TORCH_SIPU_FOUND_MMA_DTE TRUE)
message(STATUS "MMA DTE root: ${MMA_DTE_TILE_TENSOR_ROOT}")
message(STATUS "MMA DTE include dirs: ${TORCH_SIPU_MMA_DTE_INCLUDE_DIRS}")
message(STATUS "MMA DTE mode: header-only")
```

注意 `TORCH_SIPU_MMA_DTE_LIBRARY` 被**强制置空**。torch 只从 mma_dte_tile_tensor 取三个 include 目录（`:95-98`）：

```cmake
set(TORCH_SIPU_MMA_DTE_INCLUDE_DIRS
    "${MMA_DTE_TILE_TENSOR_ROOT}/include"
    "${MMA_DTE_TILE_TENSOR_ROOT}/kernel"
    "${MMA_DTE_TILE_TENSOR_ROOT}/kernel/util")
```

它甚至**不读** mma_dte_tile_tensor 的 `CMakeLists.txt`（那个文件在 `third_party/mma_dte_tile_tensor/CMakeLists.txt` 里静静躺着，torch 只 include 了它自己的 `cmake/MmaDteConfig.cmake`，用途见证据三）。

**证据二 —— torch 自己的 `.su` 做显式模板实例化。** torch 的 `torch_sipu/CMakeLists.txt:90-92` 用 glob 收集自己的 `.su`：

```cmake
file(GLOB_RECURSE SU_SRCS CONFIGURE_DEPENDS
  csrc/aten/native/sipu/*.su
  csrc/contrib/native/sipu/*.su)
```

`csrc/contrib/native/sipu/mma_dte/` 下有 129 个 `.su`。以 MXFP6 的一个为例，`BlasMmaDteMxR32S4x1M32Mxfloat6e3m2.su:1-28`：

```cpp
#include <torch_sipu/csrc/contrib/native/sipu/mma_dte/BlasMmaDte.suh>
#include <mma_dte_tiled_tensor_kernel_util_mxint8_mxfp6_r32.hpp>
#include <torch_sipu/csrc/contrib/native/sipu/mma_dte/BlasMmaDteMxKernel.suh>

namespace at::native::detail {
void mx_mma_dte_s1x4_m32_mxfloat6e3m2_fp32(
    const at::Tensor& A, const at::Tensor& B, at::Tensor& C) {
  run_mx_mma_dte_tiled_tensor<
      sifmt::mxfloat6e3m2, sifmt::sifmt::float32,
      MxMmaDteLayout::kS1x4M32Fp6>(A, B, C);
}
}
```

（上面 `sifmt::sifmt::float32` 是我为可读性改写的，原文是 `sifmt::float32`。）这些是**真正的模板实例化**，会展开出设备代码。`run_mx_mma_dte_tiled_tensor` 定义在 `mma_dte/BlasMmaDteMxKernel.suh:84`，最终在 `:66` 调用 SDK 的 `mma_dte<...>()`。

**证据三 —— torch 自己做了 host/device 两遍编译。** `torch_sipu/CMakeLists.txt:336-362` 遍历 `SU_SRCS`，为每个 `.su` 产出**两个** object：

```cmake
set(HOST_OBJ ${CMAKE_CURRENT_BINARY_DIR}/sipu_objs/${SU_UNIQUE_NAME}-host-x86_64.o)
set(DEVICE_OBJ ${CMAKE_CURRENT_BINARY_DIR}/sipu_objs/${SU_UNIQUE_NAME}-sipu-sipu150.o)
```

`:131-160` 用 SDK 的 `nds64le-elf-newlib-v5d/bin/clang++` 和 `ld.lld` 做设备侧编译（"SDK offload new driver workflow"）。

**这里有一个对你后续实现极其重要的坑**，torch 在 `:297-305` 用注释写明了：

> The mma_dte dispatch and planner are target-ISA sensitive: on the 1.5 target the MXINT8/MXFP8/MXFP6 fixed tile-register loaders are PROCON_V2 only, so SELF_PRO_SELF_CON and its SMEM kernels must not be selected. Device passes get `__SIPU_ARCH__` from the compiler, but a host pass (`--target=x86_64-unknown-linux-gnu`) never does, so the target arch must be passed explicitly. Without it the two passes of the same `.su` disagree: the host planner selects SELF_PRO_SELF_CON kernels that the device pass never instantiated, and the launch fails with "Requested function not found".

也就是说：**同一个 `.su` 的 host pass 和 device pass 必须看到一致的 arch 宏**，否则 host 侧 planner 会选出一个 device 侧根本没实例化的 kernel，运行时直接报 `Requested function not found`。torch 为此 include 了 `cmake/MmaDteConfig.cmake` 并调用 `mma_dte_add_target_arch_compile_definition()`（`:311-313`）来统一。

**结论：方案 a 确认。** 但这也意味着——如果 simo 想「自己编 mma_dte」，就必须**完整复刻**这套 host/device 两遍编译 + arch 一致性机制，工作量不小。这正是为什么我推荐下面的「复用 torch kernel」方案。

---

### 68.4 方案总览：三条路线，推荐路线 B

先说一个在调研中发现的重要事实：**simo 已经在用 mma_dte 了**。

`simo/quantization/nn/modules/sipu_linear.py:37-63` 的 `hp_to_tile_mx` 构造 `MXTileTensor`，`:112-114` 直接调 `torch.mm(mx_input, mx_weight.transpose(-2, -1))`，再 `torch.ops.aten_ext.tileformat_to_linear` 解回 linear。**这就是一条完整的 mma_dte MX matmul 调用，而且没有用任何 CMake。**（不过它当前有个版本门禁问题，见 68.5。）

所以你的需求本质上是：**把这条已有的 Python 路径，提升为一个正式的 `torch.ops.simo.mma_dte` C++ 算子**。三条候选路线：

| | 路线 A：纯 Python 包装 | **路线 B：host-only C++，复用 torch kernel（推荐）** | 路线 C：自编设备代码 |
|---|---|---|---|
| simo 侧要写什么 | 一个 Python 函数 | 一个 `.cpp`（host）+ 一个 CMakeLists | `.cpp` + `.su` + CMake + arch 一致性机制 |
| 需要 CMakeLists.txt | 不需要 | **需要** | 需要 |
| 需要编译设备代码 | 不需要 | **不需要** | 需要（scc + ndk clang 两遍） |
| 需要 pybind11 | 不需要 | 不需要（torch 自带） | 不需要（torch 自带） |
| 需要 tvm-ffi | 不需要 | 不需要 | 不需要 |
| 需要额外链接的库 | 无 | `libtorch_sipu.so` + SDK 的 `libsipu.so`/`libsipurt.so` | 还要 `libtile_mma_dte.so` 或自编 |
| fp32 输出 | 不可达（见 68.6） | **可达**（直接调 C++） | 可达 |
| 构建时间 | 0 | 秒级 | 分钟级 |
| 与 torch 版本耦合 | 弱 | **强**（符号名/ABI 绑定） | 弱 |

**推荐路线 B**，理由有三：

1. **它满足你的全部功能要求**，包括 fp32 输出（这是路线 A 做不到的，见 68.6）。
2. **它把「设备代码编译」这个最大的复杂度整个消掉了**——直接复用 torch 已经编好、已经在跑、已经过测试的 kernel。
3. **它和你给的 hpc-ops 参照物形态一致**（`TORCH_LIBRARY_FRAGMENT` + setup.py 驱动），但比 hpc-ops 轻得多——hpc-ops 要编 CUDA、要按 arch 出多个 `.so`，而你这里一行设备代码都不用编。

---

### 68.5 关键事实：torch wheel 已经导出了头文件和全部 MX 符号

这是路线 B 成立的基础，我逐条验证过。

**(1) 头文件在 wheel 里。** `site-packages/torch_sipu/include/` 下：

```
torch_sipu/csrc/contrib/native/sipu/mma_dte/BlasMmaDte.suh
torch_sipu/csrc/contrib/native/sipu/mma_dte/BlasMmaDteMxKernel.suh
torch_sipu/csrc/aten/OpSIPUType.h
c10/sipu/SIPUStream.h
```

`BlasMmaDte.suh` 只依赖 `ATen/core/Tensor.h`、`c10/core/ScalarType.h`、`OpSIPUType.h`，**不依赖任何 device-only 头**。真正需要 device 头的是 `BlasMmaDteMxKernel.suh`（它 `#include <mma_dte_tiled_tensor.hpp>`），但那个头只用于**编译期推导 layout 常量**——而如果你直接调用已经实例化好的叶子符号，连 `BlasMmaDteMxKernel.suh` 都不需要。

**(2) 符号全部导出。** `nm -D --defined-only` 显示 `libtorch_sipu.so` 导出了完整的 MX 叶子符号矩阵，命名规律是 `at::native::detail::mx_mma_dte_<几何>_<dtype>[_<outdtype>]`：

```
mx_mma_dte_s1x4_m32_mxint8            mx_mma_dte_s1x4_m32_mxint8_f16    mx_mma_dte_s1x4_m32_mxint8_fp32
mx_mma_dte_s1x4_m32_mxfloat8e4m3      ..._f16                          ..._fp32
mx_mma_dte_s1x4_m32_mxfloat8e5m2      ..._f16                          ..._fp32
mx_mma_dte_s1x4_m32_mxfloat6e3m2      ..._f16                          ..._fp32
mx_mma_dte_s1x4_m32_mxfloat6e2m3      ..._f16                          ..._fp32
mx_mma_dte_s1x4_m32_mxint4            ..._f16                          ..._fp32
mx_mma_dte_s1x4_m32_mxfloat4e2m1      ..._f16                          ..._fp32
```

以及 `s2x2_m32` / `s4x1_m32` / `s1x4_m16` / `s1x4_m8` 各系列。**你要的 5 种 MX 类型（mxint8 / mxfp8 / mxfp6 / mxfp4 / mxint4）× 三种输出 dtype（bf16 / fp16 / fp32）全部齐备。** 这些符号都是全局 `T`（非 `W`、非 local），可以被外部链接。

**(3) 顶层入口也在。** `at::native::mx_mma_dte_tiled_tensor(MxMmaDteConfig, MxMmaDteConfig, c10::ScalarType, const at::Tensor&, const at::Tensor&, at::Tensor&)` 同样是导出的 `T` 符号。它的内部是一个 `switch (self_dispatch_id)`，覆盖 35 个 dispatch id（`BlasMmaDte.su:140-213`）。

**(4) 编译可行性已实测。** 我写了一个 host-only 的探针（保存在 `like-useful/design_mma_dte_simo/reuse_probe.cpp`），只 include wheel 里的头文件、只链接 `libtorch_sipu.so`，验证了 `MxMmaDteLayout`、`MxMmaDteConfig`、`mx_mma_dte_config_spec()`、`mx_mma_dte_output_layout_spec()` 都可以正常使用。**唯一需要注意的坑是 include 顺序**：

> SiPU 的 `c10/macros/Macros.h:3` 用了 `#include_next <c10/macros/Macros.h>`，所以 **`-I<wheel>/include` 必须排在 upstream torch 的 include 路径之前**，否则 `C10_SIPU_API` 不会被定义，报 `'C10_SIPU_API' does not name a type`。

我把这条写进了 `like-useful/design_mma_dte_simo/README.md`。

---

### 68.6 一个必须知道的约束：fp32 输出在 Python 层被禁掉了

你要求 `output tensor dtype = bf16 or fp16 or fp32`。这里有一个坑：

**底层完整支持 fp32。** `BlasMmaDte.suh:325-340` 的 `mx_mma_dte_output_layout_spec()`：

```cpp
if (out_dtype == at::kHalf || out_dtype == at::kBFloat16) {
  return {512 / row_tile_elem, row_tile_elem};
}
if (out_dtype == at::kFloat) {
  return {256 / row_tile_elem, row_tile_elem};   // <- fp32 有独立规格
}
```

`BlasMmaDteMxR32.su:15` 等处也都有 `if (out_dtype == at::kFloat)` 分支，`_fp32` 叶子符号也全部导出。

**但 torch 的 Python 公开层主动禁用了它。** `mx.cpp:810-814`：

```cpp
const auto output_dtype = out_dtype.value_or(at::kBFloat16);
TORCH_CHECK(
    output_dtype == at::kBFloat16 || output_dtype == at::kHalf,
    "_mx_mm only supports bfloat16 or float16 output, got ",
    output_dtype);
```
...
```
out_dtype=torch.bfloat16: OK -> torch.bfloat16
out_dtype=torch.float16:  OK -> torch.float16
out_dtype=torch.float32:  FAIL ValueError: MX mm only supports output dtypes
                          {torch.bfloat16, torch.float16}, but got torch.float32
```

`MXTileTensor.__torch_dispatch__` 在 `mx_tensor.py:1876-1886` 也有一层同样的检查。

**这对方案的含义：**

- **路线 A（纯 Python 包装 `torch.mm`）无法满足 fp32 要求**——你只能拿到 bf16 或 fp16。
- **路线 B 可以满足**——因为你在 C++ 里直接调 `mx_mma_dte_tiled_tensor`，绕开了 `_mx_mm` 那层 `TORCH_CHECK`。这是选路线 B 的一个硬性理由。
...
按你的期望，注册为 `torch.ops.simo.mma_dte`：

```cpp
// schema（放进 simo/csrc/torch_bindings.cpp 现有的 TORCH_LIBRARY_FRAGMENT(simo, m) 里）
m.def(
    "mma_dte(Tensor a, Tensor b, int a_layout, int b_layout, int mx_dtype, "
...
| 参数 | 含义 |
|---|---|
| `a` | device=`sipu` 的 MX tile format tensor（由 siinfer `hp_to_mx` 或 simo 自己的 `hp_to_tile_mx` 产出），`scale_axis=1` |
| `b` | 同上，`scale_axis=0`（即 B 需要 `to_mx_rhs` 那种转置过的物理视图） |
| `a_layout` / `b_layout` | `MxMmaDteLayout` 的整数值（0..9），见 68.7.3 |
| `mx_dtype` | `at::MXDType` 的整数值（0=mxfloat6e3m2, 1=mxfloat6e2m3, 2=mxint8, 3=mxfloat8e4m3, 4=mxfloat8e5m2, 5=mxint4, 6=mxfloat4e2m1） |
| `out_dtype` | `torch.bfloat16` / `torch.float16` / `torch.float32`，缺省 bf16 |
| 返回 | **linear layout** 的 `[M, N]` 普通 tensor，dtype = `out_dtype` |

**注意返回的是 linear**——因为 `mx_mma_dte_tiled_tensor` 本身就是写 `at::empty({M,N})`（`mx.cpp:402` 的 `new_mx_mma_result`），输出天然就是 linear，不需要再 `tileformat_to_linear`。这正好符合你的要求，也是它比 `BlasMmaDteMxKernel.suh` 那条路更适合你的原因——后者写死了 tile format 输出。

#### 68.7.2 C++ 实现骨架

新增一个 `simo/csrc/quantization/sipu/mma_dte.cpp`（放在 `simo/csrc/` 下任意子目录，`setup.py:81` 的递归 glob 会自动收集）：

```cpp
#include <c10/macros/Macros.h>          // 必须在最前，见 68.5 的 include 顺序坑
#include <c10/util/Exception.h>
#include <c10/sipu/SIPUStream.h>
#include <ATen/ATen.h>
#include <torch/extension.h>

// torch 已编好的 host 侧头（wheel 里带了）
#include <torch_sipu/csrc/contrib/native/sipu/mma_dte/BlasMmaDte.suh>

namespace {

at::Tensor mma_dte_simo(
    const at::Tensor& a, const at::Tensor& b,
    int64_t a_layout, int64_t b_layout, int64_t mx_dtype,
    std::optional<at::ScalarType> out_dtype) {
  const auto out = out_dtype.value_or(at::kBFloat16);
  const auto cfg_a = at::native::MxMmaDteConfig{
      static_cast<at::native::MxMmaDteLayout>(a_layout),
      static_cast<at::MXDType>(mx_dtype)};
  const auto cfg_b = at::native::MxMmaDteConfig{
      static_cast<at::native::MxMmaDteLayout>(b_layout),
      static_cast<at::MXDType>(mx_dtype)};

  // 形状与 layout 合法性交给 torch 已有的校验
  const auto M = a.size(0);
  const auto N = b.size(1);
  auto result = at::empty({M, N}, a.options().dtype(out));

  // 直接调用 torch 已实例化好的 kernel（无需自己编译任何设备代码）
  at::native::mx_mma_dte_tiled_tensor(cfg_a, cfg_b, out, a, b, result);
  return result;
}
...
TORCH_LIBRARY_IMPL(simo, PrivateUse1, m) {
  m.impl("mma_dte", &mma_dte_simo);
}
```

三个要点：

1. **`TORCH_LIBRARY_IMPL(simo, PrivateUse1, m)`** —— SiPU 的 dispatch key 是 `PrivateUse1`（我在 `torch_sipu/csrc/sipu/Module.cpp:55` 等处确认，且 simo 自己在 `simo/ops/mx_api.py:428` 已经用过 `dispatch_key="PrivateUse1"`）。**不要用 `CPU`** —— 那样 SiPU tensor 根本 dispatch 不过来。
2. **schema 和 impl 分离**，这是 simo 现有约定：schema 放 `torch_bindings.cpp:18` 的 `TORCH_LIBRARY_FRAGMENT(simo, m)`，impl 放新 `.cpp`。只加 schema 会得到一个「注册了但没有实现」的 op，只加 impl 会在加载时因为 schema 未知而失败。
3. **`at::native::MxMmaDteConfig` / `mx_mma_dte_tiled_tensor` 需要在头文件里可见** —— `BlasMmaDte.suh:69` 定义了 `MxMmaDteConfig`，`:1165` 声明了 `mx_mma_dte_tiled_tensor`，都能直接用。

#### 68.7.3 layout 常量的取值

`MxMmaDteLayout` 定义在 `BlasMmaDte.suh:9-20`，序号 0..9：

```
kS4x1M32Fp6=0, kS2x2M32Fp6=1, kS1x4M32Fp6=2, kS1x4M16Fp6=3, kS1x4M8Fp6=4,
kS4x1M32=5,    kS2x2M32=6,    kS1x4M32=7,    kS1x4M16=8,    kS1x4M8=9
```

约束（`BlasMmaDte.suh:223-364` 的 `mx_mma_dte_config_spec()` 一族函数）：**A 可以是任意一个，B 必须是 M32 行 tile，且等于 `mx_mma_dte_config_spec(A).required_rhs_layout`**。具体映射：A ∈ {S4x1M32\*, S1x4M32\*} → B 同 layout；A = S2x2M32\* → B = S2x2M32\*；A ∈ {S1x4M16\*, S1x4M8\*} → B = S1x4M32\*。

Python 侧可以从 `torch_sipu.sipu.tensor.mx_tensor` 拿现成的常量：`a.tile_layout.mx_mma_layout` 和 `a.tile_layout.required_rhs_layout`（`mx_tensor.py:640-648`）。

#### 68.7.4 构建接线

**需要的 CMakeLists.txt**（这是对你「需要增加 CMakeLists.txt 吗」的直接回答）：**需要，但很小**。原因不是 simo 的 host 代码需要 CMake，而是：

- `setup.py:59` 的 `extension = CUDAExtension if use_cuda else CppExtension` 走的是 `torch.utils.cpp_extension`，它会把所有源文件当**普通 x86 C++** 编译，**不认 `.su`，也不会传 SDK 的 include/链接参数**；
- 路线 B 虽然不编设备代码，但仍需要**额外的 include 路径**（wheel 的 include、SDK 的 4 个 include 目录）和**额外的链接库**（`libtorch_sipu.so` + SDK 的 `libsipu.so` / `libsipurt.so`）。

有两种做法：

**做法 1（改动最小）：不引入 CMake，扩展 `setup.py` 的 `get_extensions()`。** 在 `setup.py:88` 附近给 `include_dirs` 和 `extra_link_args` 补上 SiPU 分支：

```python
if backend == "sipu":
    sdk = os.environ["SI_SDK_ROOT"]
    si = os.environ["TORCH_SIPU_INSTALL_PREFIX"]  # 或从 torch_sipu.__file__ 推导
    include_dirs += [
        f"{si}/include",
        f"{sdk}/include", f"{sdk}/include/SiTe",
        f"{sdk}/include/sipu_dev_apis", f"{sdk}/include/sipurt",
    ]
    extra_link_args += [f"-L{si}/lib", "-ltorch_sipu",
                        f"-L{sdk}/lib", "-lsipurt", "-lsipu"]
    extra_compile_args["cxx"] += [f"-I{si}/include"]   # 注意顺序：SiPU 在前
```

注意 `setup.py:58-59` 那个 **陷阱**：`backend == "sipu"` 时 `use_cuda` 为 False，走 `CppExtension`，**`.cu` 文件不会被编译**。好在你这条路线全是 `.cpp`，不受影响；但如果将来要加 `.cu`，必须改这里。

**做法 2（更规范，也更接近 hpc-ops）：引入 CMakeLists.txt + `CMakeExtension`。** 即照搬 hpc-ops 的 `setup.py:17-27`（`CMakeExtension` + `CMakeBuild`），CMake 里 `add_library(simo_C MODULE ...)` 并 `target_link_libraries(... torch torch_cpu c10 torch_sipu sipurt sipu)`。

**我的建议是做法 1**，因为它最小、改动集中在 `setup.py` 一个文件，而且完全避免了 CMake 与 setuptools 的耦合。**只有当你决定走路线 C（自己编设备代码）时，才必须上 CMake** —— 因为 `.su` 的 host/device 两遍编译和 arch 一致性机制用 `CppExtension` 表达不出来。

---

### 68.8 依赖清单（直接回答你的三个问题）

| 依赖 | 需要吗 | 说明 |
|---|---|---|
| **CMakeLists.txt** | 路线 B：**可选**（推荐改 `setup.py`）；路线 C：**必须** | 见 68.7.4 |
| **tvm-ffi** | **完全不需要** | 全链路零引用 |
| **pybind11** | **不需要显式引入** | `torch/include/pybind11/` 自带；simo 现有 `torch_bindings.cpp:1` 就是这么用的，`pyproject.toml` 里没声明 |
| 设备代码编译（scc） | 路线 B：**不需要** | 复用 torch 已编好的 kernel |
| `libtile_mma_dte.so` + 40 个 component DSO | **不需要** | torch 走 header-only，MX kernel 已静态编进 `libtorch_sipu.so` |
| 额外链接库 | `libtorch_sipu.so`、`libsipu.so`、`libsipurt.so` | 后两个来自 SDK |

**最小依赖方案就是路线 B。** 净新增：一个 `.cpp`、`setup.py` 里十几行 include/link 配置、schema 里一行 `m.def`。**零设备代码、零 CMake（可选）、零新第三方依赖。**

---

### 68.9 三个必须先解决的现实问题

在动手之前，有三件事会直接卡住你，我在调研中都实测碰到了：

**问题一：siinfer 的 `hp_to_mx` 输出的是 tiled 格式，但 `mma_dte` 的 A/B 需要 tiled —— 这点是吻合的；但 siinfer 没有 inverse，且两个库之间没有任何 glue 代码。**

siinfer `hp_to_mx`（`siinfer/quantization.py:10-42`）返回的是「连续一维 `torch.uint8` 物理 buffer，包含 MX metadata 和 packed data」，输出**始终**是 supertile 交织的 tiled 格式（`hp_to_mx_tensormap.hpp:103-112` 固定 4 tiles/supertile）。这与 `mma_dte` 的 A/B（`tensor_format=1`）方向一致。**但我 grep 了两个 code base，`simma.h`/`mma_dte` 在 si-infer 里零引用，`nm -D -u` 显示 siinfer 的 `.so` 对 mma_dte 零未定义引用——两者从未被接起来过。** 而且 siinfer 的 `input_layout` 参数控制的是**输入**编码，输出永远是 tiled；它**没有** `mx_to_hp` 反向函数。所以「siinfer 产出 → mma_dte 消费」这条链路是**未经验证的**，需要你先做一次端到端验证。

**问题二：simo 现有的 `sipu_linear` 路径当前是坏的。**

`simo/quantization/nn/modules/sipu_linear.py:11-20` 有个版本门禁：

```python
def _check_torch_version():
  required_version = "2.7.1+cpu.cxx11.abi"
  current_version = torch.__version__
  return current_version.endswith(required_version.split("+")[1]) and importlib.import_module("torch_sipu")

if _check_torch_version():
  from torch_sipu.sipu.mx.mx_linear import MXLinear, ...
  def hp_to_tile_mx(...): ...
```

当前环境是 `torch 2.10.0+cpu`，`"2.10.0+cpu".endswith("cpu.cxx11.abi")` 为 `False`，**整个 `if` 块被跳过**。我实测：

```
module attrs: ['ModuleQuantizeConfig', 'ScaleModeEnum', 'downcast_to_mxfmt', 'dtypes',
               'importlib', 'padding_to_mx_tileformat', 'sipu_linear',
               'to_supertile_format', 'torch']
has hp_to_tile_mx: False
has MXLinear: False
```

也就是说，`sipu_linear` 这个函数虽然还在，但它依赖的 `hp_to_tile_mx` 和 `MXLinear` **都没有被定义**——一旦真的走到 device 分支就会 `NameError`。另外 `MXLinear` 还依赖 `torchao`（`mx_linear.py:4` `from torchao.core.config import AOBaseConfig`），而 `vllm_dev` 环境里**没装 torchao**。

**这对方案的含义**：你新写的 `torch.ops.simo.mma_dte` 不应该依赖 `MXLinear`，而应该直接走 `mx_mma_dte_tiled_tensor`（这正是路线 B 的做法）。但输入张量的构造（`hp_to_tile_mx` 那条 `downcast_to_mxfmt → padding_to_mx_tileformat → to_supertile_format → MXTileTensor` 链）需要你重新梳理，因为它的顶层入口当前是死的。

**问题三：M 太小时结果不对。**

我在验证 torch 的 MX mm 时发现，`M=64/32/16` 时 `MXINT8` 与 CPU 参考实现 `rtol=0, atol=0` 完全一致，但 **`M=8` 时不一致**（maxdiff 达 146）。建议 M 取 16 以上。这个限制来自 `mx_mma_dte_config_spec()` 里 M8/M16 的 `required_rhs_layout` 与 tile 几何的耦合，不是 bug，但你的接口文档里应该写明。

---

### 68.10 分阶段实施建议

**第 0 阶段（先做，成本最低，风险最高）：端到端可行性验证。** 用 Python 拼一条最小链路，确认 siinfer 的 `hp_to_mx` 输出能被 `mma_dte` 消费：

```python
import torch, torch_sipu, siinfer
from siinfer.quantization import hp_to_mx
from torch_sipu.sipu.tensor.mx_tensor import MXTileTensor, to_mx_rhs, DTYPE_MX_INT8

DEV = "sipu"
a_hp = torch.randn(64, 512, dtype=torch.bfloat16, device=DEV)
buf = hp_to_mx(a_hp.unsqueeze(0), "mxint8", input_layout="linear")   # -> uint8 物理 buffer
# 用 buf 构造 MXTileTensor，再 torch.mm(...)
```

**这一步必须先跑通**，否则后面全是空中楼阁。如果 siinfer 的输出和 `MXTileTensor` 期望的物理布局对不上，你需要写一个 repack（参考 `simo/utils/tileformat.py:383` 的 `to_supertile_format`）。

**第 1 阶段：C++ 算子。** 按 68.7.2 写 `mma_dte.cpp`，先在 **bf16 输出**上跑通（避开 fp32 的额外不确定性），用 `MXTileTensor.to_mx` 产出的张量做输入，与 `torch.mm` 的结果对齐验证。

**第 2 阶段：打开 fp32 输出。** 加 `out_dtype=torch.float32` 支持。注意 fp32 的 output layout spec 是 `{256/row_tile, row_tile}`（`BlasMmaDte.suh:336-338`），与 bf16 的 `{512/row_tile, row_tile}` 不同，N 对齐要求也不同。

**第 3 阶段：接 siinfer。** 把 `hp_to_mx` 的产出接进来，替换第 1 阶段用的 `MXTileTensor.to_mx`。

**第 4 阶段（可选）：决定要不要走路线 C。** 只有当路线 B 的版本耦合（依赖 `libtorch_sipu.so` 的具体符号名）成为问题时才考虑。走路线 C 要复刻 torch 的 host/device 两遍编译 + arch 一致性机制（见 68.3 证据三），工作量显著更大。

---

### 68.11 参照物评价：hpc-ops 值得借鉴什么

你问 hpc-ops 有没有值得参考的。**有两点值得借鉴，但有两点不适用。**

**值得借鉴：**

1. **`TORCH_LIBRARY_FRAGMENT` 里 schema + impl 一起注册的写法**（`hpc-ops/src/group_gemm/entry.cc:397-437`），比 simo 现在 schema/impl 分两处更紧凑。不过 simo 的分离写法（schema 在 `torch_bindings.cpp`，impl 在按 dispatch key 分的 `.cpp`/`.cu`）本身没有问题，且已经支持 CPU/CUDA 双后端——**建议 simo 保持自己的约定**，只加 `PrivateUse1` 分支。
2. **`setup.py` 里 `CMakeExtension` + 自定义 `build_ext` 驱动 CMake 的模式**（`hpc-ops/setup.py:17-27, 56-63`）。如果将来 simo 确实需要 CMake（路线 C），这是标准范式。

**不适用：**

1. **hpc-ops 按 arch 出多个 `.so`**（`_C_sm90/_C_sm100/_C_sm103.abi3.so`，`hpc-ops/CMakeLists.txt:115-157`）—— 这是 CUDA 的多架构需求。SiPU 这里只需要 1.5 一个 target（`SI_CMODEL_HW_ARCH`），不需要这套。
2. **hpc-ops 编 CUDA device code**（`enable_language(CUDA)`、`find_package(CUDAToolkit)`）。你的路线 B 完全不需要编 device code，所以 hpc-ops 最复杂的那部分对你不适用。

**另外纠正一个前提**：你说 hpc-ops「通过 setup.py+CMakeLists.txt 完成构建」——这是对的；但它**没有**链接任何外部 device 库（我 grep 了 `deepgemm|sikernel|sgl-kernel`，零命中；`grep -rn dlopen src/` 也是零命中）。它的 `target_link_libraries` 只有 `cuda c10 torch torch_cpu cudart c10_cuda torch_cuda`，第三方依赖只有 header-only 的 CUTLASS。所以「hpc-ops 如何链接外部 device 库」这个参考点其实不存在。

---

### 68.12 附：本次调研产出的文件

- `like-useful/design_mma_dte_simo/reuse_probe.cpp` —— host-only 复用 torch kernel 的可行性探针
- `like-useful/design_mma_dte_simo/README.md` —— 正确的 include 顺序与链接参数（含 `#include_next` 那个坑）

---

### 68.13 一句话总结

**最小依赖方案 = 路线 B：simo 侧只写一个 host `.cpp`，直接调用 `libtorch_sipu.so` 里已经编好的 `at::native::mx_mma_dte_tiled_tensor`，输出 linear、支持 bf16/fp16/fp32。** 不需要 tvm-ffi，不需要显式 pybind11，不需要设备代码编译，不需要 `libtile_mma_dte.so` 及其 component DSO。CMakeLists.txt 可选（推荐改 `setup.py`），只有在走「自己编设备代码」的路线 C 时才必须引入。**动手前请先完成 68.9 的三个前置验证**，尤其是 siinfer → mma_dte 这条从未被接通过的链路。
