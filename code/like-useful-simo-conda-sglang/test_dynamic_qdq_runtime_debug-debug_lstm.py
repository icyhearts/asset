import json
from functools import cache
import os
import pathlib
import tempfile

import numpy as np
import onnx
import pytest

from simo.onnx.onnx_quant import apply_qdq_quantization
from simo.onnx.runtime import (
  get_custom_ops_library_path,
  register_custom_ops,
)
from simo.quantization.config import parse_quantize_spec

try:
  import onnxruntime as ort
except ImportError:
  ort = None

try:
  from onnxruntime.quantization.qdq_loss_debug import (
    compute_signal_to_quantization_noice_ratio as _ort_compute_sqnr,
  )
except (ImportError, AttributeError):
  _ort_compute_sqnr = None

try:
  import torch
  from simo.ops.flex_api import (
    per_block_downcast_to_fp8_or_int8,
    per_block_upcast,
    per_channel_downcast_to_fp8_or_int8,
    per_channel_upcast,
    per_group_downcast_to_fp8_or_int8_or_int4,
    per_group_upcast,
  )
  from simo.ops.fake_quant import fake_quantize_mx
  from simo.ops.formats.flexpoint.scale import ScaleModeEnum as FlexScaleModeEnum
  from simo.ops.formats.mx.scale import ScaleModeEnum
  from simo.ops.kernels.mx_trition_api import _downcast_to_mxfmt_triton, _upcast_from_mxfmt_triton
  from simo.quantization.dtypes import as_dtype
except ImportError:
  torch = None
  fake_quantize_mx = None
  per_block_downcast_to_fp8_or_int8 = None
  per_block_upcast = None
  per_channel_downcast_to_fp8_or_int8 = None
  per_channel_upcast = None
  per_group_downcast_to_fp8_or_int8_or_int4 = None
  per_group_upcast = None
  FlexScaleModeEnum = None
  ScaleModeEnum = None
  _downcast_to_mxfmt_triton = None
  _upcast_from_mxfmt_triton = None
  as_dtype = None


_runtime_build_directory = None


@cache
def _simo_custom_ops_library():
  lib = os.environ.get("SIMO_ONNX_CUSTOM_OPS_LIBRARY")
  if lib:
    return pathlib.Path(lib)
  try:
    return get_custom_ops_library_path()
  except RuntimeError:
    from simo.onnx.ort_plugin.build_runtime import build_sm90_runtime

    global _runtime_build_directory
    _runtime_build_directory = tempfile.TemporaryDirectory(prefix="simo-onnx-test-")
    return build_sm90_runtime(
      pathlib.Path(_runtime_build_directory.name) / "libSimoOnnxCustomOps_sm90.so"
    )


def _session_options_with_simo_plugin():
  if ort is None:
    pytest.skip("onnxruntime is not available")
  if "CUDAExecutionProvider" not in ort.get_available_providers():
    pytest.skip("CUDAExecutionProvider is not available")
  if torch is None or not torch.cuda.is_available():
    pytest.skip("No CUDA device is available")

  lib_path = _simo_custom_ops_library()
  if not lib_path.exists():
    raise FileNotFoundError(f"SIMO ONNX custom ops library does not exist: {lib_path}")

  options = ort.SessionOptions()
  return register_custom_ops(options, library_path=lib_path)


def _create_cuda_session(model, options=None):
  return ort.InferenceSession(
    model.SerializeToString(),
    sess_options=options if options is not None else _session_options_with_simo_plugin(),
    providers=["CUDAExecutionProvider"],
  )


def compute_sqnr(float_tensor, qdq_tensor):
  if _ort_compute_sqnr is not None:
    return _ort_compute_sqnr(float_tensor, qdq_tensor)

  signal = np.asarray(float_tensor, dtype=np.float64)
  noise = signal - np.asarray(qdq_tensor, dtype=np.float64)
  signal_power = np.sum(signal * signal)
  noise_power = np.sum(noise * noise)

  if noise_power == 0:
    return np.inf
  if signal_power == 0:
    return -np.inf
  return 10 * np.log10(signal_power / noise_power)


def _semantic_attrs(dtype, granularity, **attrs):
  spec_data = {"dtype": dtype}
  if "scale_mode" in attrs and attrs["scale_mode"] is not None:
    spec_data["scale_mode"] = attrs["scale_mode"]
  elif dtype == "nvfp4_e2m1":
    spec_data["scale_mode"] = "e4m3"
  if granularity == "per_channel":
    spec_data["axis"] = attrs.get("axis", 0)
  elif granularity == "per_group":
    spec_data["axis"] = attrs.get("axis", -1)
    if "group_size" in attrs:
      spec_data["group_size"] = attrs["group_size"]
  elif granularity == "per_block":
    spec_data["axis"] = attrs.get("axes", [0, 1])
    if "group_size" in attrs:
      spec_data["group_size"] = attrs["group_size"]
  if "block_size" in attrs:
    spec_data["block_size"] = attrs["block_size"]
  spec = parse_quantize_spec(spec_data)

  semantic_attrs = {
    "scale_mode": getattr(spec, "scale_mode", "fp32"),
    "group_size": int(getattr(spec, "group_size", None) or 1),
    "block_size": int(getattr(spec, "block_size", None) or 1),
  }
  if dtype in {"int8", "int4"}:
    default_min = -128.0 if dtype == "int8" else -8.0
    default_max = 127.0 if dtype == "int8" else 7.0
    semantic_attrs["quant_min"] = float(attrs.get("quant_min", default_min))
    semantic_attrs["quant_max"] = float(attrs.get("quant_max", default_max))
  return semantic_attrs


def _reference_mx_qdq(dtype, tensor, block_size, scale_mode=None, *, return_scale_dtype=False):
  if torch is None:
    pytest.skip("torch is not available")
  if scale_mode is None:
    scale_mode = ScaleModeEnum.E4M3 if dtype == "nvfp4_e2m1" else ScaleModeEnum.E8M0_FLOOR
  torch_tensor = torch.from_numpy(tensor).cuda()
  q_tensor, scale = _downcast_to_mxfmt_triton(
    torch_tensor,
    as_dtype(dtype),
    axis=-1,
    block_size=block_size,
    quant_scale_rounding_mode=scale_mode,
  )
  scale_dtype = scale.dtype
  output = torch.empty_like(torch_tensor)
  _upcast_from_mxfmt_triton(q_tensor, scale, output, as_dtype(dtype), axis=-1)
  torch.cuda.synchronize()
  if return_scale_dtype:
    return output.cpu().numpy(), scale_dtype
  return output.cpu().numpy()


def _reference_mx_qdq_with_last_dim_padding(dtype, tensor, block_size, scale_mode=None):
  pad = (-tensor.shape[-1]) % block_size
  rank2 = tensor.reshape(-1, tensor.shape[-1])
  if pad:
    rank2 = np.pad(rank2, ((0, 0), (0, pad)))
  output = _reference_mx_qdq(dtype, rank2.astype(np.float32, copy=False), block_size, scale_mode)
  return output[:, : tensor.shape[-1]].reshape(tensor.shape)


def _reference_fake_quant_mx(dtype, tensor, block_size, scale_mode):
  if torch is None or fake_quantize_mx is None:
    pytest.skip("torch fake quant is not available")
  torch_tensor = torch.from_numpy(tensor).cuda()
  output = fake_quantize_mx(
    torch_tensor,
    as_dtype(dtype),
    axis=-1,
    block_size=block_size,
    scale_mode=scale_mode,
  )
  torch.cuda.synchronize()
  return output.cpu().numpy()


def _reference_fp8_per_group_qdq(tensor, group_size):
  if torch is None:
    pytest.skip("torch is not available")
  torch_tensor = torch.from_numpy(tensor).cuda()
  q_tensor, scale = per_group_downcast_to_fp8_or_int8_or_int4(
    torch_tensor,
    as_dtype("fp8_e4m3"),
    quant_min=-448.0,
    quant_max=448.0,
    axis=-1,
    group_size=group_size,
    scale_mode=FlexScaleModeEnum.FP32,
  )
  output = per_group_upcast(q_tensor, scale, torch.float32, group_size=group_size)
  torch.cuda.synchronize()
  return output.cpu().numpy()


def _reference_fp8_per_block_qdq(tensor, group_size):
  if torch is None:
    pytest.skip("torch is not available")
  torch_tensor = torch.from_numpy(tensor).cuda()
  q_tensor, scale = per_block_downcast_to_fp8_or_int8(
    torch_tensor,
    as_dtype("fp8_e4m3"),
    quant_min=-448.0,
    quant_max=448.0,
    axis=[0, 1],
    group_size=group_size,
    scale_mode=FlexScaleModeEnum.FP32,
  )
  output = per_block_upcast(q_tensor, scale, torch.float32, group_size=group_size)
  torch.cuda.synchronize()
  return output.cpu().numpy()


def _reference_int8_per_block_qdq(tensor, group_size):
  if torch is None:
    pytest.skip("torch is not available")
  torch_tensor = torch.from_numpy(tensor).cuda()
  q_tensor, scale = per_block_downcast_to_fp8_or_int8(
    torch_tensor,
    as_dtype("int8"),
    quant_min=-128.0,
    quant_max=127.0,
    axis=[0, 1],
    group_size=group_size,
    scale_mode=FlexScaleModeEnum.FP32,
  )
  output = per_block_upcast(q_tensor, scale, torch.float32, group_size=group_size)
  torch.cuda.synchronize()
  return output.cpu().numpy()


def _reference_fp8_per_channel_qdq(tensor):
  if torch is None:
    pytest.skip("torch is not available")
  torch_tensor = torch.from_numpy(tensor).cuda()
  q_tensor, scale = per_channel_downcast_to_fp8_or_int8(
    torch_tensor,
    as_dtype("fp8_e4m3"),
    quant_min=-448.0,
    quant_max=448.0,
    axis=0,
    scale_mode=FlexScaleModeEnum.FP32,
  )
  output = per_channel_upcast(q_tensor, scale, torch.float32)
  torch.cuda.synchronize()
  return output.cpu().numpy()


def _reference_int8_per_channel_qdq(tensor):
  if torch is None:
    pytest.skip("torch is not available")
  torch_tensor = torch.from_numpy(tensor).cuda()
  q_tensor, scale = per_channel_downcast_to_fp8_or_int8(
    torch_tensor,
    as_dtype("int8"),
    quant_min=-128.0,
    quant_max=127.0,
    axis=0,
    scale_mode=FlexScaleModeEnum.FP32,
  )
  output = per_channel_upcast(q_tensor, scale, torch.float32)
  torch.cuda.synchronize()
  return output.cpu().numpy()


def _reference_int4_per_group_dequant(q_uint8, scale_uint8, logical_shape, group_size):
  if torch is None:
    pytest.skip("torch is not available")
  q_tensor = torch.from_numpy(q_uint8.view(np.int32)).cuda()
  scale_tensor = torch.from_numpy(scale_uint8.view(np.float32)).cuda()
  output = per_group_upcast(
    q_tensor, scale_tensor, torch.float32, group_size=group_size, quant_dtype=as_dtype("int4")
  )
  torch.cuda.synchronize()
  return output.cpu().numpy().reshape(logical_shape)


def _require_simo_native_reference():
  try:
    import simo._C  # noqa: F401
  except ModuleNotFoundError as exc:
    if exc.name != "simo._C":
      raise
    pytest.skip("simo._C is required for this numerical reference")


def _apply_qdq_quantization_with_native_weight_quant(model, config_path):
  _require_simo_native_reference()
  return apply_qdq_quantization(model, config_path)


def create_simo_activation_matching(qdq_activations, float_activations=None):
  pre_suffix = "_SimoQuantInput"
  post_suffix = "_SimoDequantOutput"
  matches = {}

  for name, pre_qdq in qdq_activations.items():
    if not name.endswith(pre_suffix):
      continue

    base_name = name[: -len(pre_suffix)]
    post_name = f"{base_name}{post_suffix}"
    if post_name not in qdq_activations:
      continue

    match = {
      "pre_qdq": pre_qdq,
      "post_qdq": qdq_activations[post_name],
    }
    if float_activations is not None and base_name in float_activations:
      match["float"] = float_activations[base_name]
    matches[base_name] = match

  return matches


def test_simo_activation_matching_pairs_dynamic_qdq_names():
  pre = np.array([1.0, 2.0], dtype=np.float32)
  post = np.array([1.0, 1.9], dtype=np.float32)
  floating = np.array([1.0, 2.1], dtype=np.float32)

  matches = create_simo_activation_matching(
    {
      "layer_0_SimoQuantInput": pre,
      "layer_0_SimoDequantOutput": post,
      "layer_1_SimoQuantInput": np.array([3.0], dtype=np.float32),
      "unrelated": np.array([4.0], dtype=np.float32),
    },
    {"layer_0": floating},
  )

  assert list(matches) == ["layer_0"]
  assert matches["layer_0"]["pre_qdq"] is pre
  assert matches["layer_0"]["post_qdq"] is post
  assert matches["layer_0"]["float"] is floating
  assert np.isfinite(compute_sqnr(pre, post))


def _tiny_simo_qdq_model(
  tmp_path,
  shape,
  dtype="mxint8",
  block_size=32,
  scale_mode=None,
  tensor_dtype=onnx.TensorProto.FLOAT,
  dequantize_op="Dequantize",
):
  input_name = "input"
  q_name = "input_SimoQuantInput"
  output_name = "input_SimoDequantOutput"

  graph = onnx.helper.make_graph(
    [
      onnx.helper.make_node(
        "Quantize",
        [input_name],
        [q_name, "input_SimoScale"],
        domain="com.simo",
        dtype=dtype,
        granularity="per_group",
        **_semantic_attrs(dtype, "per_group", block_size=block_size, scale_mode=scale_mode),
      ),
      onnx.helper.make_node(
        dequantize_op,
        [q_name, "input_SimoScale"],
        [output_name],
        domain="com.simo",
        dtype=dtype,
        granularity="per_group",
        **_semantic_attrs(dtype, "per_group", block_size=block_size, scale_mode=scale_mode),
      ),
    ],
    "tiny_simo_qdq",
    [onnx.helper.make_tensor_value_info(input_name, tensor_dtype, shape)],
    [onnx.helper.make_tensor_value_info(output_name, tensor_dtype, shape)],
  )
  model = onnx.helper.make_model(
    graph,
    opset_imports=[
      onnx.helper.make_operatorsetid("", 18),
      onnx.helper.make_operatorsetid("com.simo", 2),
    ],
  )
  return _create_cuda_session(model), input_name, output_name


def _dynamic_conv_activation_qdq_model(tmp_path):
  weight = (np.arange(128, dtype=np.float32).reshape(4, 32, 1, 1) - 64.0) / 32.0
  graph = onnx.helper.make_graph(
    [onnx.helper.make_node("Conv", ["X", "W"], ["Y"], name="conv")],
    "dynamic_conv_activation_qdq",
    [onnx.helper.make_tensor_value_info("X", onnx.TensorProto.FLOAT, ["N", 32, "H", "W"])],
    [onnx.helper.make_tensor_value_info("Y", onnx.TensorProto.FLOAT, ["N", 4, "H", "W"])],
    [onnx.numpy_helper.from_array(weight, "W")],
  )
  model = onnx.helper.make_model(graph, opset_imports=[onnx.helper.make_operatorsetid("", 18)])
  config_path = tmp_path / "dynamic_conv_quant_config.json"
  config_path.write_text(
    json.dumps({
      "module_configs": [
        {
          "targets": ["Conv"],
          "input": {"dtype": "mxint8", "axis": 1},
          "weight": {"dtype": "mxint8", "axis": 1, "block_size": 32},
        }
      ],
    })
  )
  model_with_qdq = _apply_qdq_quantization_with_native_weight_quant(model, config_path)
  assert any(
    node.domain == "com.simo" and node.op_type == "Quantize" for node in model_with_qdq.graph.node
  )
  return _create_cuda_session(model_with_qdq)


def _qdq_unaligned_matmul_activation_qdq_model(tmp_path, input_shape, input_config):
  weight = (np.arange(72, dtype=np.float32).reshape(18, 4) - 36.0) / 64.0
  graph = onnx.helper.make_graph(
    [onnx.helper.make_node("MatMul", ["X", "W"], ["Y"], name="matmul")],
    "unaligned_matmul_activation_qdq",
    [onnx.helper.make_tensor_value_info("X", onnx.TensorProto.FLOAT, input_shape)],
    [onnx.helper.make_tensor_value_info("Y", onnx.TensorProto.FLOAT, None)],
    [onnx.numpy_helper.from_array(weight, "W")],
  )
  model = onnx.helper.make_model(graph, opset_imports=[onnx.helper.make_operatorsetid("", 18)])
  config_path = tmp_path / "unaligned_matmul_quant_config.json"
  config_path.write_text(
    json.dumps({
      "module_configs": [
        {
          "targets": ["MatMul"],
          "input": input_config,
          "weight": {"dtype": "fp8_e4m3", "axis": [0, 1], "group_size": 128},
        }
      ],
    })
  )
  model_with_qdq = _apply_qdq_quantization_with_native_weight_quant(model, config_path)
  model_with_qdq.graph.output.append(
    onnx.helper.make_tensor_value_info(
      "matmul_input_simo_restore", onnx.TensorProto.FLOAT, [2, 3, 18]
    )
  )
  return _create_cuda_session(model_with_qdq)


def _lstm_runtime_source(direction="forward", hidden_size=4, input_size=18):
  num_directions = 2 if direction == "bidirectional" else 1
  weight = np.linspace(
    -0.75,
    0.75,
    num_directions * 4 * hidden_size * input_size,
    dtype=np.float32,
  ).reshape(num_directions, 4 * hidden_size, input_size)
  recurrent = np.linspace(
    -0.5,
    0.5,
    num_directions * 4 * hidden_size * hidden_size,
    dtype=np.float32,
  ).reshape(num_directions, 4 * hidden_size, hidden_size)
  graph = onnx.helper.make_graph(
    [
      onnx.helper.make_node(
        "LSTM",
        ["X", "W", "R"],
        ["Y", "Y_h", "Y_c"],
        name="lstm",
        hidden_size=hidden_size,
        direction=direction,
      )
    ],
    f"lstm_{direction}",
    [onnx.helper.make_tensor_value_info("X", onnx.TensorProto.FLOAT, ["S", "B", input_size])],
    [
      onnx.helper.make_tensor_value_info(
        "Y", onnx.TensorProto.FLOAT, ["S", num_directions, "B", hidden_size]
      ),
      onnx.helper.make_tensor_value_info(
        "Y_h", onnx.TensorProto.FLOAT, [num_directions, "B", hidden_size]
      ),
      onnx.helper.make_tensor_value_info(
        "Y_c", onnx.TensorProto.FLOAT, [num_directions, "B", hidden_size]
      ),
    ],
    [onnx.numpy_helper.from_array(weight, "W"), onnx.numpy_helper.from_array(recurrent, "R")],
  )
  return onnx.helper.make_model(graph, opset_imports=[onnx.helper.make_operatorsetid("", 18)]), weight, recurrent


def _reference_lstm_mx_weights(weight, recurrent, hidden_size, input_size):
  def quantize_gates(array):
    return np.stack(
      [
        np.concatenate(
          [
            _reference_mx_qdq_with_last_dim_padding(
              "mxint8", array[direction, gate * hidden_size : (gate + 1) * hidden_size, :], 32
            )
            for gate in range(4)
          ],
          axis=0,
        )
        for direction in range(array.shape[0])
      ],
      axis=0,
    )

  del input_size  # The source arrays carry the two logical K dimensions.
  return quantize_gates(weight), quantize_gates(recurrent)


def _run_lstm_qdq_case(tmp_path, direction):
  hidden_size, input_size = 4, 18
  source, weight, recurrent = _lstm_runtime_source(direction, hidden_size, input_size)
  tmp_path.mkdir(parents=True, exist_ok=True)
  source_path = tmp_path / f"lstm_{direction}_before_qdq.onnx"
  qdq_path = tmp_path / f"lstm_{direction}_after_qdq.onnx"
  onnx.save(source, source_path)
  config_path = tmp_path / f"lstm_{direction}_quant_config.json"
  config_path.write_text(
    json.dumps(
      {
        "module_configs": [
          {
            "targets": ["LSTM"],
            "input": {"dtype": "mxint8", "axis": 1, "block_size": 32},
            "weight": {"dtype": "mxint8", "axis": 1, "block_size": 32},
          }
        ]
      }
    )
  )
  model = _apply_qdq_quantization_with_native_weight_quant(source, config_path)
  onnx.save(model, qdq_path)
  onnx.checker.check_model(model)

  lstm = next(node for node in model.graph.node if node.op_type == "LSTM")
  direction_count = 2 if direction == "bidirectional" else 1
  restored_names = {
    "X_restored": lstm.input[0],
    "W_restored": lstm.input[1],
    "R_restored": lstm.input[2],
  }
  for name in restored_names.values():
    model.graph.output.append(onnx.helper.make_tensor_value_info(name, onnx.TensorProto.FLOAT, None))

  weight_dq = [
    node
    for node in model.graph.node
    if node.domain == "com.simo" and node.op_type == "Dequantize" and any(
      attribute.name == "original_shape" for attribute in node.attribute
    )
  ]
  assert len(weight_dq) == 8 * direction_count
  carrier_initializers = [
    initializer
    for initializer in model.graph.initializer
    if "_simo_d" in initializer.name
    and (initializer.name.endswith("_q") or initializer.name.endswith("_scale"))
  ]
  assert len(carrier_initializers) == 16 * direction_count
  assert all(
    any(initializer.name in node.input for node in model.graph.node)
    for initializer in carrier_initializers
  )
  assert any(node.op_type == "Pad" for node in model.graph.node)
  assert any(node.op_type == "Slice" for node in model.graph.node)

  session = _create_cuda_session(model)
  reference_weight, reference_recurrent = _reference_lstm_mx_weights(
    weight, recurrent, hidden_size, input_size
  )
  reference = onnx.ModelProto()
  reference.CopyFrom(source)
  for initializer, values in zip(
    reference.graph.initializer,
    (reference_weight, reference_recurrent),
    strict=True,
  ):
    initializer.CopyFrom(onnx.numpy_helper.from_array(values, initializer.name))
  reference_session = _create_cuda_session(reference)

  for sequence_length, batch_size in ((3, 2), (5, 1)):
    tensor = np.linspace(
      -1.0,
      1.0,
      sequence_length * batch_size * input_size,
      dtype=np.float32,
    ).reshape(sequence_length, batch_size, input_size)
    reference_input = _reference_mx_qdq_with_last_dim_padding("mxint8", tensor, 32)
    reference_outputs = reference_session.run(None, {"X": reference_input})
    actual_outputs = session.run(None, {"X": tensor})
    actual = dict(zip([output.name for output in model.graph.output], actual_outputs, strict=True))
    expected = dict(zip([output.name for output in reference.graph.output], reference_outputs, strict=True))

    assert actual["Y"].shape == (sequence_length, direction_count, batch_size, hidden_size)
    assert actual["Y_h"].shape == (direction_count, batch_size, hidden_size)
    assert actual["Y_c"].shape == (direction_count, batch_size, hidden_size)
    for name in ("Y", "Y_h", "Y_c"):
      assert actual[name].dtype == np.float32
      assert np.isfinite(actual[name]).all()
      np.testing.assert_allclose(actual[name], expected[name], rtol=3e-3, atol=3e-3)
    np.testing.assert_allclose(actual[restored_names["X_restored"]], reference_input, rtol=3e-3, atol=3e-3)
    np.testing.assert_allclose(actual[restored_names["W_restored"]], reference_weight, rtol=3e-3, atol=3e-3)
    np.testing.assert_allclose(actual[restored_names["R_restored"]], reference_recurrent, rtol=3e-3, atol=3e-3)


def test_lstm_qdq_runtime_matches_reference_for_dynamic_sequence_and_batch(tmp_path, direction):
  _run_lstm_qdq_case(tmp_path, direction)


def main():
  temp_path = pathlib.Path("temp/test_dynamic_qdq_runtime_debug-lstm")
  temp_path.mkdir(parents=True, exist_ok=True)
  test_lstm_qdq_runtime_matches_reference_for_dynamic_sequence_and_batch(temp_path, "forward")
  test_lstm_qdq_runtime_matches_reference_for_dynamic_sequence_and_batch(
    temp_path, "bidirectional"
  )


if __name__ == "__main__":
  main()


def _qdq_fp8_per_group_tail_model(tmp_path):
  options = _session_options_with_simo_plugin()
  options.enable_mem_pattern = False
  options.enable_mem_reuse = False
  weight = (np.arange(1280, dtype=np.float32).reshape(320, 4) - 640.0) / 640.0
  graph = onnx.helper.make_graph(
    [onnx.helper.make_node("MatMul", ["X", "W"], ["Y"], name="matmul")],
    "fp8_per_group_tail",
    [onnx.helper.make_tensor_value_info("X", onnx.TensorProto.FLOAT, ["N", 320])],
    [onnx.helper.make_tensor_value_info("Y", onnx.TensorProto.FLOAT, ["N", 4])],
    [onnx.numpy_helper.from_array(weight, "W")],
  )
  model = onnx.helper.make_model(graph, opset_imports=[onnx.helper.make_operatorsetid("", 18)])
  config_path = tmp_path / "fp8_per_group_tail_quant_config.json"
  config_path.write_text(
    json.dumps({
      "module_configs": [
        {
          "targets": ["MatMul"],
          "input": {"dtype": "fp8_e4m3", "axis": 1, "group_size": 128},
          "weight": {"dtype": "fp8_e4m3", "axis": [0, 1], "group_size": 128},
        }
      ],
    })
  )
  model_with_qdq = _apply_qdq_quantization_with_native_weight_quant(model, config_path)
  assert not any(node.op_type in {"Pad", "Slice"} for node in model_with_qdq.graph.node)
  model_with_qdq.graph.output.append(
    onnx.helper.make_tensor_value_info(
      "matmul_SimoDequantOutput", onnx.TensorProto.FLOAT, ["N", 320]
    )
  )
  return _create_cuda_session(model_with_qdq, options)


def _tiny_simo_fp8_per_group_qdq_model(
  tmp_path,
  shape,
  group_size=128,
  tensor_dtype=onnx.TensorProto.FLOAT,
  dequantize_op="Dequantize",
):
  input_name = "input"
  q_name = "input_SimoQuantInput"
  output_name = "input_SimoDequantOutput"

  graph = onnx.helper.make_graph(
    [
      onnx.helper.make_node(
        "Quantize",
        [input_name],
        [q_name, "input_SimoScale"],
        domain="com.simo",
        dtype="fp8_e4m3",
        granularity="per_group",
        axis=-1,
        **_semantic_attrs("fp8_e4m3", "per_group", group_size=group_size),
      ),
      onnx.helper.make_node(
        dequantize_op,
        [q_name, "input_SimoScale"],
        [output_name],
        domain="com.simo",
        dtype="fp8_e4m3",
        granularity="per_group",
        axis=-1,
        **_semantic_attrs("fp8_e4m3", "per_group", group_size=group_size),
      ),
    ],
    "tiny_simo_fp8_per_group_qdq",
    [onnx.helper.make_tensor_value_info(input_name, tensor_dtype, shape)],
    [onnx.helper.make_tensor_value_info(output_name, tensor_dtype, shape)],
  )
  model = onnx.helper.make_model(
    graph,
    opset_imports=[
      onnx.helper.make_operatorsetid("", 18),
      onnx.helper.make_operatorsetid("com.simo", 2),
    ],
  )
  return _create_cuda_session(model), input_name, output_name


def _tiny_simo_fp8_per_block_qdq_model(
  tmp_path,
  shape,
  group_size=128,
  tensor_dtype=onnx.TensorProto.FLOAT,
  dequantize_op="Dequantize",
):
  input_name = "input"
  q_name = "input_SimoQuantInput"
  output_name = "input_SimoDequantOutput"

  graph = onnx.helper.make_graph(
    [
      onnx.helper.make_node(
        "Quantize",
        [input_name],
        [q_name, "input_SimoScale"],
        domain="com.simo",
        dtype="fp8_e4m3",
        granularity="per_block",
        axes=[0, 1],
        **_semantic_attrs("fp8_e4m3", "per_block", group_size=group_size),
      ),
      onnx.helper.make_node(
        dequantize_op,
        [q_name, "input_SimoScale"],
        [output_name],
        domain="com.simo",
        dtype="fp8_e4m3",
        granularity="per_block",
        axes=[0, 1],
        **_semantic_attrs("fp8_e4m3", "per_block", group_size=group_size),
      ),
    ],
    "tiny_simo_fp8_per_block_qdq",
    [onnx.helper.make_tensor_value_info(input_name, tensor_dtype, shape)],
    [onnx.helper.make_tensor_value_info(output_name, tensor_dtype, shape)],
  )
  model = onnx.helper.make_model(
    graph,
    opset_imports=[
      onnx.helper.make_operatorsetid("", 18),
      onnx.helper.make_operatorsetid("com.simo", 2),
    ],
  )
  return _create_cuda_session(model), input_name, output_name


def _tiny_simo_int8_per_block_qdq_model(
  tmp_path,
  shape,
  group_size=128,
  quant_min=-128.0,
  quant_max=127.0,
  tensor_dtype=onnx.TensorProto.FLOAT,
  dequantize_op="Dequantize",
):
  input_name = "input"
  q_name = "input_SimoQuantInput"
  output_name = "input_SimoDequantOutput"

  graph = onnx.helper.make_graph(
    [
      onnx.helper.make_node(
        "Quantize",
        [input_name],
        [q_name, "input_SimoScale"],
        domain="com.simo",
        dtype="int8",
        granularity="per_block",
        axes=[0, 1],
        **_semantic_attrs(
          "int8",
          "per_block",
          group_size=group_size,
          quant_min=quant_min,
          quant_max=quant_max,
        ),
      ),
      onnx.helper.make_node(
        dequantize_op,
        [q_name, "input_SimoScale"],
        [output_name],
        domain="com.simo",
        dtype="int8",
        granularity="per_block",
        axes=[0, 1],
        **_semantic_attrs(
          "int8",
          "per_block",
          group_size=group_size,
          quant_min=quant_min,
          quant_max=quant_max,
        ),
      ),
    ],
    "tiny_simo_int8_per_block_qdq",
    [onnx.helper.make_tensor_value_info(input_name, tensor_dtype, shape)],
    [
      onnx.helper.make_tensor_value_info(output_name, tensor_dtype, shape),
      onnx.helper.make_tensor_value_info(q_name, onnx.TensorProto.UINT8, shape),
    ],
  )
  model = onnx.helper.make_model(
    graph,
    opset_imports=[
      onnx.helper.make_operatorsetid("", 18),
      onnx.helper.make_operatorsetid("com.simo", 2),
    ],
  )
  return _create_cuda_session(model), input_name, output_name, q_name


def _tiny_simo_fp8_per_channel_qdq_model(
  tmp_path,
  shape,
  axis=0,
  tensor_dtype=onnx.TensorProto.FLOAT,
  dequantize_op="Dequantize",
):
  input_name = "input"
  q_name = "input_SimoQuantInput"
  output_name = "input_SimoDequantOutput"

  graph = onnx.helper.make_graph(
    [
      onnx.helper.make_node(
        "Quantize",
        [input_name],
        [q_name, "input_SimoScale"],
        domain="com.simo",
        dtype="fp8_e4m3",
        granularity="per_channel",
        axis=axis,
        **_semantic_attrs("fp8_e4m3", "per_channel"),
      ),
      onnx.helper.make_node(
        dequantize_op,
        [q_name, "input_SimoScale"],
        [output_name],
        domain="com.simo",
        dtype="fp8_e4m3",
        granularity="per_channel",
        axis=axis,
        **_semantic_attrs("fp8_e4m3", "per_channel"),
      ),
    ],
    "tiny_simo_fp8_per_channel_qdq",
    [onnx.helper.make_tensor_value_info(input_name, tensor_dtype, shape)],
    [onnx.helper.make_tensor_value_info(output_name, tensor_dtype, shape)],
  )
  model = onnx.helper.make_model(
    graph,
    opset_imports=[
      onnx.helper.make_operatorsetid("", 18),
      onnx.helper.make_operatorsetid("com.simo", 2),
    ],
  )
  return _create_cuda_session(model), input_name, output_name


def _tiny_simo_int8_per_channel_qdq_model(
  tmp_path,
  shape,
  quant_min=-128.0,
  quant_max=127.0,
  tensor_dtype=onnx.TensorProto.FLOAT,
  dequantize_op="Dequantize",
):
  input_name = "input"
  q_name = "input_SimoQuantInput"
  output_name = "input_SimoDequantOutput"

  graph = onnx.helper.make_graph(
    [
      onnx.helper.make_node(
        "Quantize",
        [input_name],
        [q_name, "input_SimoScale"],
        domain="com.simo",
        dtype="int8",
        granularity="per_channel",
        axis=0,
        **_semantic_attrs("int8", "per_channel", quant_min=quant_min, quant_max=quant_max),
      ),
      onnx.helper.make_node(
        dequantize_op,
        [q_name, "input_SimoScale"],
        [output_name],
        domain="com.simo",
        dtype="int8",
        granularity="per_channel",
        axis=0,
        **_semantic_attrs("int8", "per_channel", quant_min=quant_min, quant_max=quant_max),
      ),
    ],
    "tiny_simo_int8_per_channel_qdq",
    [onnx.helper.make_tensor_value_info(input_name, tensor_dtype, shape)],
    [
      onnx.helper.make_tensor_value_info(output_name, tensor_dtype, shape),
      onnx.helper.make_tensor_value_info(q_name, onnx.TensorProto.UINT8, shape),
    ],
  )
  model = onnx.helper.make_model(
    graph,
    opset_imports=[
      onnx.helper.make_operatorsetid("", 18),
      onnx.helper.make_operatorsetid("com.simo", 2),
    ],
  )
  return _create_cuda_session(model), input_name, output_name, q_name


def _tiny_simo_int4_per_group_dequant_model(
  tmp_path,
  logical_shape,
  group_size=32,
  tensor_dtype=onnx.TensorProto.FLOAT,
  dequantize_op="Dequantize",
  quant_min=-8.0,
  quant_max=7.0,
):
  q_name = "Q"
  scale_name = "S"
  output_name = "Y"
  packed_shape = [logical_shape[0], logical_shape[1] // 8 * 4]
  scale_shape = [logical_shape[0], logical_shape[1] // group_size * 4]
  semantic_attrs = _semantic_attrs(
    "int4",
    "per_group",
    group_size=group_size,
    quant_min=quant_min,
    quant_max=quant_max,
  )

  graph = onnx.helper.make_graph(
    [
      onnx.helper.make_node(
        dequantize_op,
        [q_name, scale_name],
        [output_name],
        domain="com.simo",
        dtype="int4",
        granularity="per_group",
        axis=-1,
        **semantic_attrs,
      ),
    ],
    "tiny_simo_int4_per_group_dequant",
    [
      onnx.helper.make_tensor_value_info(q_name, onnx.TensorProto.UINT8, packed_shape),
      onnx.helper.make_tensor_value_info(scale_name, onnx.TensorProto.UINT8, scale_shape),
    ],
    [onnx.helper.make_tensor_value_info(output_name, tensor_dtype, logical_shape)],
  )
  model = onnx.helper.make_model(
    graph,
    opset_imports=[
      onnx.helper.make_operatorsetid("", 18),
      onnx.helper.make_operatorsetid("com.simo", 2),
    ],
  )
  return _create_cuda_session(model), q_name, scale_name, output_name


def test_simo_custom_qdq_plugin_rejects_unsupported_mxint8_shape(tmp_path):
  session, input_name, output_name = _tiny_simo_qdq_model(tmp_path, [2, 2])
  tensor = np.array([[-1.0, 0.0], [0.5, 1.0]], dtype=np.float32)

  with pytest.raises(Exception, match="K % 32 == 0"):
    session.run([output_name], {input_name: tensor})


@pytest.mark.parametrize(
  ("dtype", "block_size"),
  [
    ("mxint8", 32),
    ("mxfp8_e5m2", 32),
    ("mxfp8_e4m3", 32),
    ("mxfp6_e3m2", 32),
    ("mxfp6_e2m3", 32),
    ("mxfp4_e2m1", 32),
    ("nvfp4_e2m1", 16),
  ],
)
def test_simo_custom_qdq_plugin_runs_mx_qdq_tiny_tensor(tmp_path, dtype, block_size):
  session, input_name, output_name = _tiny_simo_qdq_model(
    tmp_path,
    [128, 32],
    dtype=dtype,
    block_size=block_size,
  )
  tensor = np.linspace(-4.0, 4.0, 128 * 32, dtype=np.float32).reshape(128, 32)

  (actual,) = session.run([output_name], {input_name: tensor})

  assert actual.shape == tensor.shape
  assert actual.dtype == np.float32
  assert np.isfinite(compute_sqnr(tensor, actual))
  np.testing.assert_allclose(
    actual,
    _reference_mx_qdq(dtype, tensor, block_size),
    rtol=1e-3,
    atol=1e-3,
  )


def test_simo_custom_qdq_plugin_preserves_float16_io_dtype(tmp_path):
  session, input_name, output_name = _tiny_simo_qdq_model(
    tmp_path,
    [128, 32],
    tensor_dtype=onnx.TensorProto.FLOAT16,
    dequantize_op="DequantizeFloat16",
  )
  tensor = np.linspace(-4.0, 4.0, 128 * 32, dtype=np.float16).reshape(128, 32)

  (actual,) = session.run([output_name], {input_name: tensor})

  expected = _reference_mx_qdq("mxint8", tensor.astype(np.float32), 32).astype(np.float16)
  assert actual.dtype == np.float16
  np.testing.assert_allclose(actual, expected, rtol=1e-3, atol=1e-3)


def test_simo_custom_qdq_plugin_preserves_bfloat16_io_dtype(tmp_path):
  session, input_name, output_name = _tiny_simo_qdq_model(
    tmp_path,
    [128, 32],
    tensor_dtype=onnx.TensorProto.BFLOAT16,
    dequantize_op="DequantizeBFloat16",
  )
  tensor = torch.linspace(-4.0, 4.0, 128 * 32, device="cuda", dtype=torch.bfloat16).reshape(128, 32)
  actual = _run_cuda_iobinding(session, {input_name: tensor}, output_name)

  expected = torch.from_numpy(_reference_mx_qdq("mxint8", tensor.float().cpu().numpy(), 32)).to(
    device="cuda", dtype=torch.bfloat16
  )
  assert actual.dtype == torch.bfloat16
  torch.testing.assert_close(actual, expected, rtol=1e-3, atol=1e-3)


def _run_cuda_iobinding(session, feeds, output_names):
  single_output = isinstance(output_names, str)
  output_names = [output_names] if single_output else list(output_names)
  binding = session.io_binding()
  for name, value in feeds.items():
    binding.bind_ortvalue_input(name, ort.OrtValue.from_dlpack(value))
  for output_name in output_names:
    binding.bind_output(output_name, "cuda")
  session.run_with_iobinding(binding)
  outputs = [torch.from_dlpack(output) for output in binding.get_outputs_as_ortvaluevector()]
  return outputs[0] if single_output else outputs


@pytest.mark.parametrize(
  ("tensor_dtype", "dequantize_op", "torch_dtype"),
  [
    (onnx.TensorProto.FLOAT, "Dequantize", torch.float32),
    (onnx.TensorProto.FLOAT16, "DequantizeFloat16", torch.float16),
    (onnx.TensorProto.BFLOAT16, "DequantizeBFloat16", torch.bfloat16),
  ],
  ids=("fp32", "fp16", "bf16"),
)
@pytest.mark.parametrize(
  "builder",
  [_tiny_simo_int8_per_block_qdq_model, _tiny_simo_int8_per_channel_qdq_model],
  ids=("per-block", "per-channel"),
)
@pytest.mark.parametrize(
  ("quant_min", "quant_max", "amax", "minimum_code"),
  [(-128.0, 127.0, 127.5, -128), (-127.0, 127.0, 127.0, -127)],
  ids=("full", "narrow"),
)
def test_simo_custom_int8_range_and_half_even_rounding(
  tmp_path,
  builder,
  quant_min,
  quant_max,
  amax,
  minimum_code,
  tensor_dtype,
  dequantize_op,
  torch_dtype,
):
  session, input_name, output_name, q_name = builder(
    tmp_path,
    [1, 8],
    quant_min=quant_min,
    quant_max=quant_max,
    tensor_dtype=tensor_dtype,
    dequantize_op=dequantize_op,
  )
  tensor = torch.tensor(
    [[-amax, amax, 0.5, 1.5, 2.5, -0.5, -1.5, -2.5]],
    device="cuda",
    dtype=torch_dtype,
  )

  quantized, actual = _run_cuda_iobinding(session, {input_name: tensor}, [q_name, output_name])

  expected_codes = torch.tensor(
    [[minimum_code, 127, 0, 2, 2, 0, -2, -2]], device="cuda", dtype=torch.int8
  )
  torch.testing.assert_close(quantized.view(torch.int8), expected_codes, rtol=0, atol=0)
  torch.testing.assert_close(actual, expected_codes.to(torch_dtype), rtol=0, atol=0)


@pytest.mark.parametrize(
  "builder",
  [_tiny_simo_int8_per_block_qdq_model, _tiny_simo_int8_per_channel_qdq_model],
  ids=("per-block", "per-channel"),
)
def test_simo_custom_int8_rejects_unsupported_numeric_range(tmp_path, builder):
  with pytest.raises(Exception, match="does not support.*quant_min=-126"):
    builder(tmp_path, [1, 8], quant_min=-126.0, quant_max=127.0)


@pytest.mark.parametrize(
  ("tensor_dtype", "dequantize_op", "torch_dtype"),
  [
    (onnx.TensorProto.FLOAT16, "DequantizeFloat16", torch.float16),
    (onnx.TensorProto.BFLOAT16, "DequantizeBFloat16", torch.bfloat16),
  ],
  ids=("fp16", "bf16"),
)
@pytest.mark.parametrize(
  ("builder", "shape"),
  [
    (_tiny_simo_fp8_per_group_qdq_model, [8, 128]),
    (_tiny_simo_fp8_per_block_qdq_model, [130, 129]),
    (_tiny_simo_fp8_per_channel_qdq_model, [8, 65]),
  ],
  ids=("per-group", "per-block", "per-channel"),
)
def test_simo_custom_typed_flex_qdq_wrapper_smoke(
  tmp_path, builder, shape, tensor_dtype, dequantize_op, torch_dtype
):
  session, input_name, output_name = builder(
    tmp_path,
    shape,
    tensor_dtype=tensor_dtype,
    dequantize_op=dequantize_op,
  )
  tensor = torch.linspace(-4.0, 4.0, int(np.prod(shape)), device="cuda", dtype=torch_dtype).reshape(
    shape
  )

  actual = _run_cuda_iobinding(session, {input_name: tensor}, output_name)

  assert list(actual.shape) == shape
  assert actual.dtype == torch_dtype
  assert torch.isfinite(actual).all()


@pytest.mark.parametrize(
  ("tensor_dtype", "dequantize_op", "torch_dtype"),
  [
    (onnx.TensorProto.FLOAT16, "DequantizeFloat16", torch.float16),
    (onnx.TensorProto.BFLOAT16, "DequantizeBFloat16", torch.bfloat16),
  ],
  ids=("fp16", "bf16"),
)
def test_simo_custom_typed_int4_dequantize_wrapper_smoke(
  tmp_path, tensor_dtype, dequantize_op, torch_dtype
):
  logical_shape = [3, 64]
  session, q_name, scale_name, output_name = _tiny_simo_int4_per_group_dequant_model(
    tmp_path,
    logical_shape,
    tensor_dtype=tensor_dtype,
    dequantize_op=dequantize_op,
  )
  quantized = torch.arange(24, device="cuda", dtype=torch.int32).reshape(3, 8).view(torch.uint8)
  scale = torch.full((3, 2), 0.5, device="cuda", dtype=torch.float32).view(torch.uint8)

  actual = _run_cuda_iobinding(
    session,
    {q_name: quantized, scale_name: scale},
    output_name,
  )

  assert list(actual.shape) == logical_shape
  assert actual.dtype == torch_dtype
  assert torch.isfinite(actual).all()


@pytest.mark.parametrize(
  "dtype",
  ["mxint8", "mxfp8_e5m2", "mxfp8_e4m3", "mxfp6_e3m2", "mxfp6_e2m3", "mxfp4_e2m1"],
)
def test_simo_custom_qdq_plugin_runs_mx_e8m0_sipu_tiny_tensor(tmp_path, dtype):
  block_size = 32
  session, input_name, output_name = _tiny_simo_qdq_model(
    tmp_path,
    [4, block_size],
    dtype=dtype,
    block_size=block_size,
    scale_mode="e8m0_sipu",
  )
  tensor = np.zeros((4, block_size), dtype=np.float32)
  tensor[:, 0] = np.array([1.9, 3.7, 7.5, 15.0], dtype=np.float32)
  tensor[:, 1] = -tensor[:, 0] / 3.0

  (actual,) = session.run([output_name], {input_name: tensor})
  expected, scale_dtype = _reference_mx_qdq(
    dtype,
    tensor,
    block_size,
    ScaleModeEnum.E8M0_SIPU,
    return_scale_dtype=True,
  )

  assert scale_dtype == torch.uint8

  np.testing.assert_allclose(
    actual,
    expected,
    rtol=1e-3,
    atol=1e-3,
  )


def test_simo_custom_qdq_plugin_matches_torch_fake_quant_for_kws_mxfp8_e5m2_activation(tmp_path):
  block_size = 32
  session, input_name, output_name = _tiny_simo_qdq_model(
    tmp_path,
    [4, 128],
    dtype="mxfp8_e5m2",
    block_size=block_size,
    scale_mode="e8m0_sipu",
  )
  tensor = np.linspace(-0.75, 0.75, 4 * 128, dtype=np.float32).reshape(4, 128)
  tensor[2, 106] = -0.0361
  tensor[3, 120] = 0.5

  (actual,) = session.run([output_name], {input_name: tensor})
  expected = _reference_fake_quant_mx(
    "mxfp8_e5m2",
    tensor,
    block_size,
    ScaleModeEnum.E8M0_SIPU,
  )

  np.testing.assert_allclose(
    actual,
    expected,
    rtol=1e-3,
    atol=1e-3,
  )


def test_dynamic_conv_activation_qdq_runs_with_symbolic_batch_and_spatial_dims(tmp_path):
  session = _dynamic_conv_activation_qdq_model(tmp_path)
  tensor = np.linspace(-1.0, 1.0, 2 * 32 * 5 * 7, dtype=np.float32).reshape(2, 32, 5, 7)

  (actual,) = session.run(["Y"], {"X": tensor})

  assert actual.shape == (2, 4, 5, 7)
  assert actual.dtype == np.float32
  assert np.isfinite(actual).all()


def test_symbolic_transposed_gemm_padding_preserves_output_shape(tmp_path):
  weight = (np.arange(72, dtype=np.float32).reshape(18, 4) - 36.0) / 64.0
  graph = onnx.helper.make_graph(
    [onnx.helper.make_node("Gemm", ["X", "W"], ["Y"], name="gemm", transA=1)],
    "symbolic_transposed_gemm",
    [onnx.helper.make_tensor_value_info("X", onnx.TensorProto.FLOAT, [18, "M"])],
    [onnx.helper.make_tensor_value_info("Y", onnx.TensorProto.FLOAT, ["M", 4])],
    [onnx.numpy_helper.from_array(weight, "W")],
  )
  model = onnx.helper.make_model(graph, opset_imports=[onnx.helper.make_operatorsetid("", 18)])
  config_path = tmp_path / "symbolic_transposed_gemm.json"
  config_path.write_text(
    json.dumps({
      "module_configs": [
        {
          "targets": ["Gemm"],
          "input": {"dtype": "mxint8"},
          "weight": {"dtype": "int8"},
        }
      ]
    })
  )
  model = _apply_qdq_quantization_with_native_weight_quant(model, config_path)
  session = _create_cuda_session(model)
  tensor = np.linspace(-1.0, 1.0, 18 * 5, dtype=np.float32).reshape(18, 5)

  (actual,) = session.run(["Y"], {"X": tensor})

  assert actual.shape == (5, 4)
  assert np.isfinite(actual).all()


@pytest.mark.parametrize(
  "input_shape",
  ([2, 3, 18], ["B", "S", 18], None),
  ids=("static", "symbolic", "unknown-rank"),
)
@pytest.mark.parametrize("dtype", ("mxint8", "int8"))
def test_unaligned_matmul_activation_qdq_matches_simo_torch_with_padding(
  tmp_path, input_shape, dtype
):
  session = _qdq_unaligned_matmul_activation_qdq_model(tmp_path, input_shape, {"dtype": dtype})
  tensor = np.linspace(-3.0, 3.0, 2 * 3 * 18, dtype=np.float32).reshape(2, 3, 18)

  (actual,) = session.run(["matmul_input_simo_restore"], {"X": tensor})

  assert actual.shape == tensor.shape
  expected = (
    _reference_mx_qdq_with_last_dim_padding("mxint8", tensor, 32)
    if dtype == "mxint8"
    else _reference_int8_per_channel_qdq(tensor.reshape(1, -1)).reshape(tensor.shape)
  )
  np.testing.assert_allclose(actual, expected, rtol=1e-3, atol=1e-3)


def test_simo_custom_qdq_plugin_runs_fp8_per_group_tail_without_padding(tmp_path):
  group_size = 128
  session = _qdq_fp8_per_group_tail_model(tmp_path)
  tensor = np.linspace(-3.0, 3.0, 2 * 320, dtype=np.float32).reshape(2, 320)

  (actual,) = session.run(["matmul_SimoDequantOutput"], {"X": tensor})

  assert actual.shape == tensor.shape
  np.testing.assert_allclose(
    actual,
    _reference_fp8_per_group_qdq(tensor, group_size),
    rtol=1e-3,
    atol=1e-3,
  )


def test_simo_custom_qdq_plugin_runs_fp8_per_group_tiny_tensor(tmp_path):
  group_size = 128
  session, input_name, output_name = _tiny_simo_fp8_per_group_qdq_model(
    tmp_path,
    [32, group_size],
    group_size=group_size,
  )
  tensor = np.linspace(-8.0, 8.0, 32 * group_size, dtype=np.float32).reshape(32, group_size)

  (actual,) = session.run([output_name], {input_name: tensor})

  assert actual.shape == tensor.shape
  assert actual.dtype == np.float32
  assert np.isfinite(compute_sqnr(tensor, actual))
  _require_simo_native_reference()
  np.testing.assert_allclose(
    actual,
    _reference_fp8_per_group_qdq(tensor, group_size),
    rtol=1e-3,
    atol=1e-3,
  )


def test_simo_custom_qdq_plugin_runs_fp8_per_block_tiny_tensor(tmp_path):
  group_size = 128
  session, input_name, output_name = _tiny_simo_fp8_per_block_qdq_model(
    tmp_path,
    [130, 129],
    group_size=group_size,
  )
  tensor = np.linspace(-8.0, 8.0, 130 * 129, dtype=np.float32).reshape(130, 129)

  (actual,) = session.run([output_name], {input_name: tensor})

  assert actual.shape == tensor.shape
  assert actual.dtype == np.float32
  assert np.isfinite(compute_sqnr(tensor, actual))
  _require_simo_native_reference()
  np.testing.assert_allclose(
    actual,
    _reference_fp8_per_block_qdq(tensor, group_size),
    rtol=1e-3,
    atol=1e-3,
  )


def test_simo_custom_qdq_plugin_runs_int8_per_block_tiny_tensor(tmp_path):
  group_size = 128
  session, input_name, output_name, _ = _tiny_simo_int8_per_block_qdq_model(
    tmp_path,
    [130, 129],
    group_size=group_size,
  )
  tensor = np.linspace(-8.0, 8.0, 130 * 129, dtype=np.float32).reshape(130, 129)

  (actual,) = session.run([output_name], {input_name: tensor})

  assert actual.shape == tensor.shape
  assert actual.dtype == np.float32
  assert np.isfinite(compute_sqnr(tensor, actual))
  _require_simo_native_reference()
  np.testing.assert_allclose(
    actual,
    _reference_int8_per_block_qdq(tensor, group_size),
    rtol=1e-3,
    atol=1e-3,
  )


def test_simo_custom_qdq_plugin_runs_fp8_per_channel_tiny_tensor(tmp_path):
  session, input_name, output_name = _tiny_simo_fp8_per_channel_qdq_model(tmp_path, [33, 65])
  tensor = np.linspace(-8.0, 8.0, 33 * 65, dtype=np.float32).reshape(33, 65)

  (actual,) = session.run([output_name], {input_name: tensor})

  assert actual.shape == tensor.shape
  assert actual.dtype == np.float32
  assert np.isfinite(compute_sqnr(tensor, actual))
  _require_simo_native_reference()
  np.testing.assert_allclose(
    actual,
    _reference_fp8_per_channel_qdq(tensor),
    rtol=1e-3,
    atol=1e-3,
  )


def test_simo_custom_qdq_plugin_rejects_noncanonical_per_channel_axis(tmp_path):
  with pytest.raises(Exception, match="canonical axis=0"):
    _tiny_simo_fp8_per_channel_qdq_model(tmp_path, [8, 32], axis=1)


def test_simo_custom_qdq_plugin_runs_int8_per_channel_tiny_tensor(tmp_path):
  session, input_name, output_name, _ = _tiny_simo_int8_per_channel_qdq_model(tmp_path, [3, 1025])
  tensor = np.linspace(-8.0, 8.0, 3 * 1025, dtype=np.float32).reshape(3, 1025)

  (actual,) = session.run([output_name], {input_name: tensor})

  assert actual.shape == tensor.shape
  assert actual.dtype == np.float32
  assert np.isfinite(compute_sqnr(tensor, actual))
  _require_simo_native_reference()
  np.testing.assert_allclose(
    actual,
    _reference_int8_per_channel_qdq(tensor),
    rtol=1e-3,
    atol=1e-3,
  )


@pytest.mark.parametrize(
  ("quant_min", "quant_max"), [(-8.0, 7.0), (-7.0, 7.0)], ids=("full", "narrow")
)
def test_simo_custom_dequant_plugin_runs_int4_per_group_tiny_tensor(tmp_path, quant_min, quant_max):
  group_size = 32
  logical_shape = [3, 64]
  session, q_name, scale_name, output_name = _tiny_simo_int4_per_group_dequant_model(
    tmp_path,
    logical_shape,
    group_size=group_size,
    quant_min=quant_min,
    quant_max=quant_max,
  )
  q_i32 = np.arange(logical_shape[0] * logical_shape[1] // 8, dtype=np.int32).reshape(
    logical_shape[0], logical_shape[1] // 8
  )
  scale = np.linspace(
    0.01, 0.03, logical_shape[0] * (logical_shape[1] // group_size), dtype=np.float32
  ).reshape(logical_shape[0], logical_shape[1] // group_size)
  q_uint8 = q_i32.view(np.uint8)
  scale_uint8 = scale.view(np.uint8)

  (actual,) = session.run([output_name], {q_name: q_uint8, scale_name: scale_uint8})

  assert actual.shape == tuple(logical_shape)
  assert actual.dtype == np.float32
  _require_simo_native_reference()
  np.testing.assert_allclose(
    actual,
    _reference_int4_per_group_dequant(q_uint8, scale_uint8, logical_shape, group_size),
    rtol=1e-3,
    atol=1e-3,
  )
