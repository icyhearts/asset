...
echo ">you-get done"
# 
ssh -p $PORT root@$DOWNHOST "cd $HTTP_ROOT/$MID_VIDEO_DIR && tar -cf ${FINAL_STR}.tar ${FINAL_STR} && md5sum ${FINAL_STR}.tar"  || exit 1
echo ">tar done"
#wget -c http://$DOWNHOST/${MID_VIDEO_DIR}/${FINAL_STR}.tar || exit 1
axel -n 10 -a http://$DOWNHOST/${MID_VIDEO_DIR}/${FINAL_STR}.tar || exit 1
md5sum ${FINAL_STR}.tar
