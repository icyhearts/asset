set -x
set -e
proj_dir="/data/like/package/vllm-for-conda-simo"
ssh_out=$(ssh bjh18 "cd $proj_dir; git push -f gitlab HEAD >/dev/null 2>&1; git rev-parse HEAD; git branch --show-current")
...
#"
#ssh_out=$(ssh bjh18 " cd $proj_dir; git rev-parse HEAD; git branch --show-current ")
